...
| 🔴 | [0-3c/f](#0-3-建議回覆與知識庫檢索的關鍵缺口) | RAG 檢索品質可否調校？有無相關度分數？（問「電梯困人」未命中對應 SOP） |
...
| 🟠 | [0-3h](#0-3-建議回覆與知識庫檢索的關鍵缺口) | 知識庫檢索需要 13～25 秒，可否調校？（直接影響建議卡能否引用 SOP） |
...
我們的介面會顯示建議回覆卡（SOP 來源＋可直接送出的回覆全文）與知識庫自然語言快查。已自行驗證：agent 能透過 SSE 取得引用來源（檔名＋chunk 原文，見附錄）、能穩定回傳 JSON。**剩下的缺口**：

| # | 問題 |
|---|---|
| 0-3c 🔴 | 有無**相關度分數**？實測 RAG 工具輸出無 score 欄位 |
| 0-3f 🔴 **最優先** | **RAG 檢索品質可否調校？** 問「電梯困人的處理步驟」，未命中同名的 `金融大樓電梯困人SOP.pdf`，反而回傳「管理辦法」的火災段落。chunk 大小／top-k／中文斷詞／同義詞可否調整？**2026-08-27 追加證據**：三個語意完全不同的查詢（「服務流程」「帳單怎麼查詢」「客訴處理原則」）全部命中**同一份文件的同三個片段**，看不出查詢字串對檢索結果有實質影響 |
| 0-3e 🟡 | 同一 agent 能否針對不同任務給不同 system prompt？ |
| 0-3h 🟠 | **知識庫檢索需要 13～25 秒，可否調校？** 我們對真實知識庫取樣九次（三個不同模型的 agent，皆確認有真的呼叫 `RAGknowledge`），最快 13.0 秒、最慢 24.9 秒，**沒有任何一次低於 13 秒**：`us.amazon.nova-pro-v1:0` 13.1／16.7／17.1／23.8 秒、`qwen.qwen3-32b-v1:0` 20.5／13.0／18.6 秒、`qwen.qwen3-vl-235b-a22b` 24.9 秒（後者連續呼叫時全面逾時，我們已不採用）。相較之下同組織不掛知識庫的 agent 單純回一句話只要 2.6～3.8 秒，所以慢的是檢索本身而不是推論。請問這個延遲可否調校（例如 chunk 數量、向量索引、top-k）？有無非同步／預先檢索的機制？**這直接影響我們的產品形態**：客服建議回覆卡必須在 10 秒內出現，13 秒以上的檢索等於建議卡永遠拿不到 SOP 引用，我們只能改成「先出無引用版本、檢索回來再更新」的兩段式設計 |
| 0-3g 🟡 | 知識庫檔案是否有可查詢的「最後修改時間」中繼資料？我們目前只能靠檔名裡的日期片段（如 `_V1_20250925_`）猜測，猜不到時前端會顯示「更新日期未知」。另外，貴司內部是否有正式的 SOP 編號制度可供串接？我們目前的介面**不**顯示任何編號（只顯示標題），若貴司確實有一套正式編號制度，我們會評估是否值得改為顯示 |
...
此題也決定我們的 AI 分析重試策略：在規格到位前，我們對 429 **一律不自動重試**（直接顯示錯誤、由客服決定何時重試），以免副駕面板的多個分析區塊各自退避重試而放大對貴方的壓力。
