...
| H-2 圖片/PDF 文字化、URL 時效、附件端點 | 🟡 | **已大幅縮小範圍**——H-2a/e 不需回覆；H-2b/c 已自行驗證（無描述、caption 僅客服上傳有值）；H-2f-b（端點範圍）已依情境自答為聯絡人層級；**H-2d（URL 時效）、H-2f（`contact/files` 端點合法性）為剩餘關鍵題** |
...
> 逐一實測 27 個後：**11 個可用**（含 4 個能做知識庫檢索）、16 個失敗，細節見下表。
> 0-3 的多數子題我們也已自行驗證出答案，詳見下方 §0-3。
...
> 若只能取得自由文字、拿不到分數，我們仍可接受降級（信心度欄位留空、只顯示 SOP 來源），但需要知道邊界在哪裡。
...
> ⚠️ **2026-08-26 更新（第二次）：已用真實對話（圖片 1 則、PDF 2 則，客服／客戶上傳各一次）自行驗證多數問題。**
>
> `image` 與 `pdf` 兩種型別的 `content` 皆帶**直接可用的原始檔 URL**，沒有描述、OCR 或轉錄欄位。
> **`caption` 有明顯規律**：客服上傳的 PDF，`caption` 是原始檔名；**客戶上傳的（圖片與 PDF 皆然）目前樣本
> `caption` 均為空**——客戶上傳才是真實場景主力，代表我們不能依賴 `caption` 顯示描述，H-2c 已可視為
> 部分自答，仍歡迎貴方確認是否為保證行為。**客戶端上傳介面本身就只接受圖片與 PDF**
> （由介面截圖確認），Word/Excel 等格式在這個管道無法上傳，H-2e 已可視為自答。
> 仍待貴方確認的只剩 **H-2d（URL 時效）與新增的 H-2f**。

**需要確認**：

| # | 項目 | 問題 |
|---|---|---|
| ~~H-2a~~ | ~~語音訊息~~ | ✅ **已自行解答，不需回覆**：我方確認 iMBrace 平台不支援語音訊息，此題不再適用 |
| H-2b | **圖片／PDF 附件** | ✅ **已自行驗證（3 樣本：圖片 1、PDF 2）**：目前沒有描述或 OCR，`content` 只有 `url`（PDF 另有 `caption`，見 H-2c）。**若貴方能提供描述／OCR，我們就能省下自建 vision／文件分析的成本**，仍歡迎告知是否有規劃 |
| H-2c | **`caption` 的語意** | 🟡 **已大致自答**：實測 2 則客服上傳的 PDF 皆為原始檔名，1 則客戶上傳的圖片／PDF 皆為空。看起來像**上傳時系統自動帶入的原始檔名**，而非使用者輸入或 AI 產生。樣本數還小，**這個「客服上傳才有檔名、客戶上傳沒有」的規律是否為保證行為？** |
| **H-2d** | **附件 URL** | 🔴 **仍待回覆，唯一關鍵題**：3 個樣本的 `content.url` 皆未加簽章、無時效參數，樣本仍小。`content.url` 是否有時效？下載是否需額外授權標頭？我們計劃直接送這個 URL 給 vision／文件分析模型，**若有時效，快取時機點的設計需要改變** |
| ~~H-2e~~ | ~~檔案型別~~ | ✅ **已自行解答**：客戶端上傳介面（web 頻道）僅接受圖片與 PDF，其餘格式傳不進來。不確定其他頻道（如 LINE、API 直送）是否有更寬鬆的限制，若貴方知道歡迎補充，但不阻塞 |
| **H-2f** | **`/contact/{id}/files` 端點：是否為正式介面？** | 🆕 **新增**：我們是在官方介面的「聯絡人資料（User Profile）」彈窗開啟時，從網路請求觀察到 `GET .../api/channel-service/v1/contact/{contact_id}/files` 會回傳附件（含 url/caption），且用我方既有 access token 即可呼叫成功。**這是否為正式支援、可依賴的介面？** 若是內部實作細節，我們不應該依賴它 |
| H-2f-b | **範圍確認（已大致自答，僅需確認）** | 🟡 **已依情境自答，歡迎指正**：由於是在「聯絡人資料」彈窗中發現，我們判斷這支端點回傳的是**聯絡人所有對話的附件，而非單一對話**——路徑本身也只有 `contact_id`、沒有 conversation id，與此判斷一致。若我們判斷錯誤（例如平台會依當前開啟的對話 context 隱性限定範圍），請務必指正，這會直接影響我們的 UI 設計 |

> **為何重要**：`image`／`pdf` 型附件已確認拿得到原始檔，我方已決定納入 vision／文件分析（自建，因平台未提供描述）。
> **剩下會顯著影響實作方式的是 H-2d 與 H-2f**——若 URL 有時效，我們必須在收到訊息當下就處理並快取；
> 若 `contact/files` 端點不是正式介面，我們需要改用其他方式列出對話附件——
> **無論如何，我方都不會把這支端點的結果當成「單一對話的附件清單」使用**（範圍判斷見 H-2f-b），
> 若真要用會定位為「客戶歷史附件」的呈現方式。
...
| 🟠 P1 | **H-2d** 圖片/PDF URL 是否有時效 | 決定 vision／文件分析何時處理與快取 | ✅ 已決定自建分析，成本比原估低（有 url，只缺描述） |
| 🟡 P2 | **H-2f** `contact/files` 端點是否為正式支援介面 | 決定「客戶歷史附件」功能能否依賴此端點 | ✅ 範圍已依情境判斷為聯絡人層級（非對話層級），不影響對話附件清單的實作 |
