...
| 🟠 | [0-3h](#0-3-建議回覆與知識庫檢索的關鍵缺口) | 知識庫檢索需要 9～20 秒，可否調校？（直接影響建議卡能否引用 SOP） |
...
| 0-3h 🟠 | **知識庫檢索需要 9～20 秒，可否調校？** 我們對現行的 `us.amazon.nova-pro-v1:0` 知識庫 agent 取樣 12 次（同一段查詢、同一時間窗，皆確認有真的呼叫 `RAGknowledge` 並回傳命中）：**最快 9.4 秒、中位 11.9 秒、p90 16.9 秒、最慢 20.1 秒**。相較之下同組織不掛知識庫的 agent 單純回一句話只要 2.6～3.8 秒，所以慢的是檢索本身而不是推論。請問這個延遲可否調校（例如 chunk 數量、向量索引、top-k）？有無非同步／預先檢索的機制？**這直接影響我們的產品形態**：客服建議回覆卡必須在 10 秒內出現，而檢索**沒有任何一次**在 8 秒內完成（0/12），等於建議卡拿不到 SOP 引用，我們只能改成「先出無引用版本、檢索回來再更新」的兩段式設計。<br>⚠️ **2026-08-29 修訂**：我們先前提供的區間是「13～25 秒、沒有任何一次低於 13 秒」，那組數字取樣過小（三個不同模型各三四次，其中 nova-pro 只有 4 個點）。加大樣本後最快值降到 9.4 秒，**我們無法排除是原樣本不足而非平台變快**，因此以新數字為準並在此說明，避免您依舊數字排查。 |
