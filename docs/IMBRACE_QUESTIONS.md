...
| H-6 送訊息的請求／回應規格 | 🟡 | 欄位名已自行確認為 `conversation_id`；仍需成功回應的物件形狀 |
...
> **為何 H-6a 重要**：我們在送出前會比對「版本錨點之後有沒有別人回覆過」，
> 以防止客戶收到重複回覆。送出成功後必須立刻把錨點推到新訊息，
> 否則自己剛送出的那則會在下一輪輪詢時被當成新訊息重複處理。
...
| 🟠 P2 | **H-6a** 送訊息成功後的回應形狀 | 撞單防護的版本錨點 | 🟡 可自行摸索，但會多一趟查詢 |
