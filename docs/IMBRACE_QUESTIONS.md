...
### 0-1. 我們組織的 AI provider 目前無法執行推論

我們對組織內 **27 個 AI Agent 逐一實測**，全部失敗，錯誤形態有三種：
...
| # | 問題 |
|---|---|
| 0-3a | agent 的回答中，能否得知它**引用了哪些來源**？（檔名、章節、chunk id 皆可） |
| 0-3b | 若能，是隨回應以結構化欄位回傳，還是只能靠 prompt 要求它寫在文字裡？ |
| 0-3c | 有無任何**相關度分數**可取得？（若無，我們會拿掉介面上的信心度數字） |
| 0-3d | 能否要求 agent 回傳**固定格式的 JSON**？（我們需要穩定解析摘要、情緒分數等欄位） |
| 0-3e | 同一個 agent 能否針對不同任務給不同 system prompt？還是每個任務需各建一個 agent？ |
