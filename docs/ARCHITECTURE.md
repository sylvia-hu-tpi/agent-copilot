...
> ### ⚠️ 剩下兩項為何還沒勾
>
> **兩者都需要「兩個真實瀏覽器 + 真實平台」才驗得出來**，自動化測試只能驗到
> 元件層級，驗不到「客服 B 的畫面上幾秒後真的出現了那則訊息」。
>
> | 未勾項目 | 已實作的部分 | 還缺什麼 |
> |---|---|---|
> | 4 秒內看到 | 第一層清單輪詢 3s（§9.3.1）+ 送出後 `poke()` 立即拉取 | 端到端計時 |
> | 斷線重連補齊 | 指數退避重連 + 重連後以 `lastMessageId` 對帳（§9.5） | 實際斷網再接回的驗證 |
>
> **在這兩項通過前，M1 不得視為完成，也不得打 `m1-done` tag。**
