...
若 004 決定放寬 15 秒門檻，判準應以第二段為準；而第二段的延遲又直接受 004 自己的設計影響
（是否重新生成整批、是否併入檢索等待），**因此 FR-014 的裁決 MUST 在 004 的設計定案後才做**，
不可先動。
...
因此 **§12.3 的快查與建議卡必須用兩個
不同的逾時值**：快查是客服主動發起、畫面有骨架的同步查詢（30 秒），建議卡走「先檢索再生成」
的串行流程且受 SC-001 門檻約束（量測時 10 秒，2026-08-29 改為 20 秒，串行仍放不進去；8 秒，逾時即以「未引用知識庫」續行）。
**已知後果：建議卡目前拿不到引用**——正解是改成漸進式（先出無引用版本、檢索回來再更新），
已另立規格承接。延遲本身已列入 `docs/IMBRACE_QUESTIONS.md` 0-3h 詢問可否調校。
