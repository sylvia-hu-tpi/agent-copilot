...
⚠️ **摘要的 M2「10 秒 p90」驗收項因此不會通過**，且成因是模型延遲、repo 內沒有槓桿補得回來
...
- [x] JOIN 後 **15 秒**內**情緒**的實質內容呈現（90 百分位）—— 實測 p90 12.7 秒（數據見 §8.2b）
...
- [ ] JOIN 後 10 秒內**摘要**的實質內容呈現（90 百分位）—— **已驗證，三輪皆不通過**（§8.2b）
...
- [ ] JOIN 後 **20 秒**內首批建議卡呈現（90 百分位）—— **三輪皆不通過**，p90 28.7／28.8／29.3 秒
...
| 驗收未通過 | 摘要 10 秒 p90、建議卡第一段 20 秒 p90 —— 皆為模型延遲，門檻**不放寬** | §18 M2 |
