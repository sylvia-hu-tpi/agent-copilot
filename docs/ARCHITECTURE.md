...
>
> 版本：v1.0 ｜ 制定日期：2026-08-24 ｜ 狀態：M2 完成（`m2-done`，2026-09-03）；`specs/006` 已合回 `main`（`m3-006-done`，2026-09-08），M3 剩餘三條標為「卡外部回覆·已安置」，M3 **保持開放**（只有 `m3-006-done`，沒有 `m3-done`），待回覆到位後回頭完成（§18 M3）；進行中的是 M3.5「SIT 展示就緒」，**不等 M3 收尾就先行**（§18 M3.5）
>
...
│                          │                                      │
│   ┌──────────────────────┴─────────────────────────────────┐   │
│   │  StateStore / EventBus  (M0–M3.5 記憶體 → M4 Redis)     │   │
│   └────────────────────────────────────────────────────────┘   │
└──────────────────────────┬────────────────────────────────────┘
                           │ @imbrace/sdk
┌──────────────────────────┼────────────────────────────────────┐
...
>
> ⚠️ **這個目標在 M3 只達成了大部分，不要照字面引用。** provider 抽象擋掉的是 webhook 與 Knowledge API 的**規格**未定，擋不掉「平台沒有內建 OCR」「沒有書面 rate limit 規格」這種**能力面**的缺口——M3 剩餘三條（附件 vision／文件分析兩條、429 全域退避佇列）正是卡在後者，2026-09-08 已標為「卡外部回覆·已安置」（§18 M3、§19.3）。M3.5 則是刻意只收「不依賴任何外部回覆就能關閉」的項目。
...
>
> ⚠️ **M3 尚未完成，MUST NOT 因為結案那幾條齊了就押 `m3-done`**：仍有四條未勾，皆不屬 `specs/006` 範圍——① Viki provider（本期不換）；②③ 附件 vision／文件分析；④ 429 全域佇列（卡 G-2 🔴）。②③④ **於 2026-09-08 決定安置**（比照 M2「未達標·已安置」的**標記與處置說明**）：三者卡的都是 `IMBRACE_QUESTIONS.md` 尚無回覆的題目，不進 M3.5、不開 007，重啟條件寫在各條。
>
> ⚠️ **但「安置」在這裡不比照 M2 的「安置後押 tag 收尾」**——M2 的三條是時效門檻，安置後功能仍完整；M3 的 ②③④ 是尚未交付的能力，安置只是「先擱著、等回覆再做」，M3 因此**保持開放**，`m3-done` 要等回覆到位、②③④ 回頭做完後才押（或由使用者另行裁定提前收尾）。**`m3-done` 不是 M3.5 的前提**：M3.5 存在的理由正是「回覆全缺，先把不卡回覆的工項推上 SIT」，所以里程碑順序會是 M3.5 先行、M3 之後回頭補完，tag 的時間順序也會跟著顛倒。不要看到 M3.5 開跑就以為 M3 已收尾——判準只有一個：`git tag -l m3-done`。M3 真正收尾時 MUST 比照 M2：綠燈基線之外另審一次跨 spec 的熱點檔案。
...
| 卡外部回覆·已安置（M3 剩餘） | 圖片／PDF 的 vision／文件分析（兩條，卡 E-3／H-2d／0-1）；429 全域退避佇列（卡 G-2）。2026-09-08 決定安置、不進 M3.5、不開 007；重啟條件＝對應題目得到回覆。M3 因此**保持開放**（無 `m3-done`），M3.5 不等它先行 | §18 M3 |
...
M0 / M1 / M3.5  地基 → 直接開發，不走 Spec Kit（避免儀式成本）
功能單元             → 走 /specify → /clarify → /plan → /tasks → /implement
                       · 情緒面板     · 建議卡與一鍵帶入   · 知識庫快查
                       · 分析觸發策略 · 漸進式引用         · M2 殘留缺陷
                       · 結案摘要與人審面板
```

⚠️ **分野是「地基或功能」，不是里程碑編號**。M3.5「SIT 展示就緒」交付的是 image 與 `deploy/` 部署組成，
沒有 user story 可寫，因此比照 M0／M1 直接開發、不開 spec（2026-09-08 決定，§18 M3.5）。

**為何適合**：Spec Kit 的 `/clarify` 階段會強制把規格缺口顯性標記出來。本專案天生就有大量未定規格（見 §19），而每個未定的外部依賴剛好對應一個 provider 介面——很乾淨的切分。

**為何不全套**：M0／M1／M3.5 本質上是地基，硬寫成 user story 會產生大量儀式性文件卻無對應的決策價值。
