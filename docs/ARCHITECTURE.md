...
| | `gemma-3-27b` | `gpt-oss-20b` |
|---|---|---|
| 中位數 | **11384ms** | 11877ms（無改善） |
| **最慢** | **16891ms** | 21204ms（+26%） |
| 超過 15 秒（整批失敗） | 1/5 | **2/5** |
| schema 合規 | 5/5 | 5/5 |
| **三筆 SOP 全數引用** | **5/5** | **3/5** ⛔ |
...
⚠️ **尚未量測它值多少毫秒**，因此不宣稱它能解決逾時；但它是目前唯一「不改規格數字、
不改模型、不改批次大小」就能削減固定成本的候選。
...
**②-2 檢索本身需要 13～25 秒，而且能不能檢索取決於「模型 ＋ prompt 措辭」兩個條件**
（2026-08-27 對真實知識庫實測，九次取樣）。兩件事分開講：
...
*要多久*：最快 13.0 秒、最慢 24.9 秒，**沒有任何一次低於 13 秒**；同組織不掛知識庫的 agent
單純回一句話只要 2.6～3.8 秒，所以慢的是檢索而非推論。因此 **§12.3 的快查與建議卡必須用兩個
