...
| `AgentCopilot_建議回覆_agent` | **`openai.gpt-oss-20b-1:0`** | 2026-08-29 由 `google.gemma-3-27b-it` 改為此值，**目的是模型比較、非最終決定**；比較結果見下方「模型比較」，其結論不支持保留此模型 |
...
| | `gemma-3-27b` | `gpt-oss-20b` |
|---|---|---|
| 中位數 | 10297ms | **7007ms**（−32%） |
| 最快 | 9094ms | **6348ms** |
| **最慢** | **11458ms** | 14926ms（尾巴反而更差 +30%） |
| 超過 10 秒 | 3/5 | **1/5** |
| schema 合規 | 5/5 | 5/5 |
...
1. **⛔ 情緒 agent 的 −48% 沒有移植過來，尾巴反而更糟。** gpt-oss 在情緒任務上的優勢是
   *範圍更窄*（4743～9210ms），但在建議卡任務上範圍**更寬**（第一段 6348～14926、
   第二段 6822～21204）。中位數改善了，但**逾時是被最慢值打死的**（本節前述原則），
   而它的最慢值兩段都輸給 gemma。第二段更是完全沒有改善。
...
3. **⛔ 換模型解不了 FR-001。** 002 SC-001 是 **p90 判準**（「90% 以上的情況能在 10 秒內」），
   不是中位數判準。兩個模型的第一段 p90 都破線（gemma ≈ 11.5 秒、gpt-oss ≈ 14.9 秒）。
   ⚠️ **且 n=5 對 p90 幾乎沒有解析度** —— 要判 p90 至少需 n≥10、理想 n≥20。
   目前的證據足以說「換模型不是解法」，**不足以說「第一段能不能達標」**。
