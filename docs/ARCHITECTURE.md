...
| `AgentCopilot_建議回覆_agent` | `google.gemma-3-27b-it` | 未變更 |
...
**建議回覆 agent 的延遲基準**（2026-08-29，`npm run spike:agent-latency -- suggestion|suggestion-kb 5`，
模型由 API 驗證為 `google.gemma-3-27b-it`）—— 這是 004 FR-001 兩段式設計的前提數據：

| | 第一段（`suggestion`，無知識庫命中） | 第二段（`suggestion-kb`，帶 3 筆命中） |
|---|---|---|
| prompt 長度 | 571 字 | 879 字 |
| 延遲範圍 | 8496～10704ms | 10170～12092ms |
| 中位數 | **9181ms** | **11432ms** |
| 超過 10 秒（002 SC-001） | **2/5** | **5/5** |
| 超過 15 秒（003 FR-014） | 0/5 | 0/5 |
| schema 合規／`sopId` 白名單 | 5/5、5/5 | 5/5、5/5（三筆命中全數正確引用，零編造） |

三個結論：

1. **兩段式是必要的，而且收益比「差 2.2 秒」大得多。** 表面上兩段只差 2251ms，但第二段
   還要**串行等知識庫檢索**，而檢索實測最快 13.1 秒、最慢 23.8 秒（見
   `agent-knowledge-provider.ts` 的 `KNOWLEDGE_SEARCH_TIMEOUT_MS` 註解）。
   第二段真正的端到端是 **24～36 秒**，第一段是 9.2 秒 —— 相差一個數量級，不是 2.2 秒。
2. **第一段以現行模型吃不下 002 SC-001 的 10 秒預算。** 9.18 秒中位、10.7 秒最慢、2/5 次
   已超線，而這只是 AI 呼叫本身：觸發判定、SSE 推送、前端渲染都還沒算進去。
   ⚠️ 且平台延遲會隨時間漂移約 36%（同節前述），10.7 × 1.36 ≈ 14.6 秒。
   **004 FR-001 的「第一段在 10 秒內完成」以 `gemma-3-27b` 不成立。**
3. **輸出品質不是問題，延遲才是。** 兩組各 5/5 合規，帶命中時三筆 SOP 全部正確引用、
   零編造 id（憲法 4.3 白名單），`requiresData` 也正確把「工單編號」交還客服。
   換模型時 MUST 重驗這兩項 —— 快而錯比慢而對更危險。

⚠️ **未量測**：把建議回覆 agent 換成 `openai.gpt-oss-20b-1:0` 後的數字。情緒 agent 換該模型
後中位數降 48%（10.5 → 5.4 秒），若比例可移植，第一段約落在 4.8 秒、最慢約 5.5 秒，
10 秒預算即有餘裕 —— 但**這是推估，不是實測**，`gemma` 在情緒任務上輸出決定性反而優於
`gpt-oss`，建議卡任務未必等比。換模型需在 iMBrace 後台操作，決定與量測見 004 的 plan。
