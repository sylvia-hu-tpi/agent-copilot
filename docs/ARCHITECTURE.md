...
`stats.lowestScore` 會跟著變。⚠️ **但示警行為不受影響** —— 示警由 label 判定
（`isSentimentAlerting()` 的遲滯規則），而兩模型合計 16 次執行的 label 序列全部正確一致。
...
本表其餘各列未重測，維持原值。詳見 §12「②-2 檢索本身需要 9～20 秒」一段的二次訂正。

⚠️ **摘要 agent 的 42.9 秒尖峰這 15 次沒有重現**（最慢 7646ms）。它曾是「FR-014 是否放寬」
的主要理由，該理由目前**沒有新證據支持**。但 n=15 也不足以排除罕見尖峰 —— 42.9 秒那次是
真實發生過的，不是量測誤差，**MUST NOT 據此認定問題已消失**。
...
**②-2 檢索本身需要 9～20 秒，而且能不能檢索取決於「模型 ＋ prompt 措辭」兩個條件**
...
- [ ] JOIN 後 **20 秒**內首批建議卡完整呈現（90 百分位；2026-08-29 由 10 秒拆行改寫——建議卡 agent 單次生成 p90 10.31 秒，10 秒在現行平台必然不過，裁決見 `specs/004-progressive-citations/spec.md` Clarifications 2026-08-29；摘要／情緒餘裕充足，不跟著放寬）
- [ ] 一鍵帶入可用，且帶入後仍會做撞單檢查
