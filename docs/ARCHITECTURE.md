...
> ⏳ **補上後尚未重測** —— 跑 `npm run spike:sentiment-dispersion` 看 24-A 的 label 翻面數是否歸零。
...
（`server/services/copilot-analysis.ts`），比照 2026-08-29 對第二段的先例，**不動 FR-014 的共用 15 秒**，
因此摘要與情緒的失敗偵測完全不受影響。
...
#### ⚠️ 單輪 n=15 判不動任何門檻 —— 相隔 30 分鐘的兩輪結論相反（2026-09-02）

同一組六個對話、同一份程式、同一組門檻，兩輪相隔約 30 分鐘：

| 驗收項 | 02:29 那輪 | 02:58 那輪 | 合併 n=30 |
|---|---|---|---|
| SC-001 首批卡 ≤20 秒 | 14/15 ＝ 93% ✅ | 12/15 ＝ 80% ❌ | **26/30 ＝ 87% ❌** |
| SC-005 摘要 ≤10 秒 | 14/15 ＝ 93% ✅ | **8/15 ＝ 53%** ❌ | **22/30 ＝ 73% ❌** |
| SC-005 情緒 ≤15 秒 | 11/15 ＝ 73% ❌ | **14/15 ＝ 93%** ✅ | **25/30 ＝ 83% ❌** |

**摘要 93% → 53%、情緒 73% → 93%，兩項在兩輪之間往相反方向大幅擺動。**
第二輪的 3 個失敗樣本全部集中在該輪第 1 圈的前三個目標，特徵極端：檢索 30 秒逾時、
第二段單次呼叫實際跑了 **102 秒**與 48 秒；第 1 圈後兩個目標即恢復正常。
排除那 3 筆後：SC-001 26/27 ＝ 96%、摘要 22/27 ＝ 81%、情緒 22/27 ＝ 81%。
...
> `summarize()` 成功時 2/2 帶齊，實例 `topics=["網路斷線","數據機"]`，
> 長度與張數都在 `SUMMARY_TOPIC_MAX_*` 上限內），不需要去後台補 prompt。
> ⚠️ **這個結論的保存期限取決於 repo 外的設定，不會有任何東西通知我們它變了。**
> 動到摘要欄位、或懷疑正文變短時重跑那支 spike —— 它刻意把 `narrative`／`topics` 分開計數、
...
- [ ] JOIN 後 **15 秒**內**情緒**的實質內容呈現（90 百分位）—— ⚠️ **2026-09-02 由通過退回未定**
      （原記載：2026-09-01 實測 p90 12.7 秒 → 打勾）。2026-09-02 兩輪合併 **25/30 ＝ 83%，未達**，
      且**兩輪本身互相矛盾**（73% / 93%），因此既不打勾也不宣告失敗（數據見 §8.2b）。
...
- [ ] JOIN 後 10 秒內**摘要**的實質內容呈現（90 百分位）—— **未通過，且成因已與原記載不同**（§8.2b）
      ...
      2026-09-02 重測：兩輪合併 22/30 ＝ 73%（單輪 93% 與 53%），排除一次降級爆發後 81%。
      仍未達，但**現在的落差是「尾巴偶爾拉到 10～14 秒」，不是原記載的「模型慢到 25 秒」**。
...
- [ ] JOIN 後 **20 秒**內首批建議卡呈現（90 百分位）—— 未通過，但**原記載的數字與成因都已作廢**
      ...
      2026-09-02 兩輪合併 26/30 ＝ 87%，排除一次降級爆發後 26/27 ＝ 96%。
...
| 未重測 | 情緒 `frustrated`／`angry` tie-breaker 補上後 label 是否還會翻面 | §8.2b |
...
> 與平台延遲都會漂移，且都不在版本控制裡）。原始輸出在 `scripts/spike/out/`。
...
### C-3 情緒 prompt 改版的離散度（2026-09-01，`spike:sentiment-dispersion`）
