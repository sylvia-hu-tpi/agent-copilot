...
sparkline 換一個形狀。2026-09-01 的實例，證據見 `scripts/spike/24-sentiment-dispersion.ts`：

**起因**：使用者回報走勢圖「分數與顏色變化滿大」。第一直覺是刻度不穩，**實測推翻**——
同一則重測擺動 ≤ 5 分、label 零次翻面、`score` 與 `label` 的分級 18/18 一致。
抖動不是雜訊，是模型對某些句子的判斷本來就那樣。

**真因是當時 system prompt 的一條規則**：「判斷依據只能是該則發言的文字內容本身」。
...
> ⚠️ **代價必須一起記，否則下次會有人以為這只賺不賠**：判斷一旦依賴上下文，
> 分數就開始依賴「**跟誰同一批**」。實測同一則訊息單獨成批與併入六則批次差到 25 分、
> `label` 還會翻。而正式路徑每 `SENTIMENT_CHUNK_SIZE`（6）則就切一次獨立呼叫，
> 於是折線在**每個批次邊界**都會出現一段不是真實情緒變化的斷層。
> **但這個斷層在 prompt 補上絕對標準之後自己消失了，不需要程式碼去補。**
> 上面那 25 分是在「只加了看上下文、還沒加分數帶與界線規則」的中間狀態量到的。
> 補上分數帶（`calm` 80–100 … `angry` 0–19）與兩條 tie-breaker 之後，同一個量測
> 掉到 **3.6 分**——模型有了絕對標準，就不再拿同批其他訊息當相對基準，
> 批次組成也就不再影響分數。
>
> ⚠️ **不要用程式碼去補這個接縫 —— 已經試過並移除了。** 2026-09-01 曾為
> `AIProvider.analyzeSentiment()` 加上 `priorPoints`（把前一批尾端的 `score`／`label`
> 帶進下一批的 prompt）。它是在**中間狀態**下加的，當時 n=3 量到「偏離 12.3 → 1.0」
> 看似大幅有效；補完 prompt 之後同一個量測是 **3.6 對 3.9，差距在雜訊內**，
> 該參數與其測試已整個移除。
>
> 留這一段是因為「批次之間沒有上下文」這個直覺很強，日後很容易有人再加一次。
> **要加之前先跑 `npm run spike:sentiment-dispersion` 看 24-B 現在是多少** ——
> 若仍是個位數，那個洞不存在，補它只是多一段要維護的程式碼（前情還帶著一條隱性
> 正確性規則：MUST 排除本批自己的訊息，否則手動重試會參考到自己上次的答案而永遠翻不了案）。

⚠️ **量測方法上的教訓**：24-B 第一版用「單一次跑的分數」當基準真相，量出「帶前情反而更差」
的結論。但那個基準值本身在 10～30 之間跳——要偵測的訊號（5～25 分）與 agent 自身的擺動
同一個量級時，**單一樣本不構成證據，兩種條件都必須取平均**。

⚠️ 而且 **n=3 也還不夠**：改成各跑 3 次之後，同一支 probe 在兩次執行間先後給出
「帶前情大幅改善」與「帶前情明顯變差」兩個**方向相反**的結論，兩次都是雜訊。
加大到 n=5 才穩定下來（兩者實質相同）。這與 §8.2b 下方「n=5 被 n=15 推翻」
是同一類錯誤，只是換了個面貌 —— **效果量與雜訊同級時，要先把 n 加大到能分辨，
再談結論**，否則量測本身會製造出方向隨機的「發現」。
...
**情緒 agent 換模型的理由**（2026-08-28，`scripts/spike/18-agent-model-latency.ts`，
兩模型各 8 次、背靠背同一時間窗、模型皆由 API 驗證）：
...
**逾時是被最慢值打死的，不是中位數。** gemma 在平台狀況良好時最慢就已到 12.7 秒，
而平台延遲實測會隨時間漂移約 36%（同模型同輸入，30 分鐘內 7.5 秒 ↔ 10.2 秒）——
12.7 × 1.36 ≈ 17.2 秒即破線，這正是 2026-08-28 真實對話上情緒分析連續兩次逾時的成因。
換模型後同一對話的全歷史冷啟動（多個 6 則批次，**當時是依序跑** —— 2026-09-01 起改為
有上限的並行，見 §11.8 ②）一次通過、零失敗。

代價是失去輸出的決定性：同一批訊息重跑會得到略為不同的分數（±10），sparkline 高度與
`stats.lowestScore` 會跟著變。

> ⚠️ **2026-09-01 修正：這一段原本接著寫「但示警行為不受影響 —— 兩模型合計 16 次執行的
> label 序列全部正確一致」，那句話現在只有一半成立，不要再照它推論。**
>
> `isSentimentAlerting()` 只分「有沒有示警」，而 `frustrated` 與 `angry` 都算示警，
> 所以**示警的開關**確實不受分數漂移影響——原句這一半仍然對。
>
> 但 `SentimentGauge` 把兩級**畫成不同東西**（`--warn` ＋ ⚠️ 圖示 對
> `--danger` ＋ 🔥 圖示，文案也不同，這是 001 FR-003 明文要求的可區分性）。
> 而 spike 24 在新 system prompt 下量到 label **真的會在這兩級之間翻面**：
> 「我要申訴，順便問一下解約要怎麼辦」三次跑出 30／30／10（`frustrated`／`frustrated`／`angry`），
> 同一則的分數擺動 20 分——大於本節記錄的 ±10。
> 也就是說**示警的「等級」會在重跑之間跳**，客服看到的圖示與顏色會在 ⚠️ 和 🔥 之間變。
>
> ⚠️ 兩組數字的量測條件不同（本節是 2026-08-28 的舊 system prompt，spike 24 是
> 2026-09-01 改版後），因此這**不是說當時量錯了**，而是說 ±10 與「label 一致」
> 都只在當時的 prompt 下成立，不能當作現行行為的依據。
>
> 成因是那類句子卡在 `frustrated`／`angry` 的界線上，而 system prompt 原本只對
> `concerned`／`frustrated` 那條界線給了讓步規則，這條線沒有 tie-breaker。
>
> **已定案（2026-09-01）**：在後台 prompt 補上「`frustrated` 與 `angry` 之間拿不定主意時
> 取 `frustrated`」。取捨是**讓真正該顯示 🔥 的少一些，換取圖示不在重跑之間跳**——
> 客服看到的示警等級穩定，比多標出幾則「更生氣」重要。
> ⚠️ 上面那組 30／30／10 是**補上這條之前**量到的；補上後尚未重測，
> 重測請跑 `npm run spike:sentiment-dispersion`，看 24-A 的 label 翻面數是否歸零。

⚠️ **`google.gemma-3-27b-it` 並非全面不可用，別把兩件事混為一談。** 它**不能**用於知識庫
檢索（缺原生 function calling，機制與判斷方式見 §13 的「能不能檢索」一節），但在**不呼叫
任何工具**的摘要／情緒／建議卡上完全正常 —— 2026-08-28 實測它在情緒 agent 上是所有候選裡
輸出最穩定的（8/8 合規、連分數都完全一致、`drivers` 中文精準），換掉它純粹是為了延遲。

**建議回覆 agent 的模型比較**（2026-08-29，`npm run spike:agent-latency -- suggestion|suggestion-kb 5`，
四組各 5 次、**同一時間窗內背靠背**、模型皆由 API 驗證）—— 004 FR-001 兩段式設計的前提數據。

⚠️ **背靠背是使用者要求的，不是多此一舉。** 先前於 10:53 量過一輪 gemma，40 分鐘後重量，
第一段中位 9181 → 10297ms（+12%）、第二段最慢 12092 → 16891ms（+40%）。**單輪數據會低估。**
下表一律採用背靠背那一輪；10:53 那輪保留為 `scripts/spike/out/*.1053.json`。

**第一段（`suggestion`，無知識庫命中，prompt 571 字）** —— 受 002 SC-001 的約束（量測時為 10 秒；**2026-08-29 已據此改為 20 秒**，見下方結論 3 的裁決註記）。
⚠️ **以 n=15 為準，n=5 那輪已被推翻**（理由見下）：
...
⚠️ **n=5 的結論是錯的，這是本次量測最重要的方法論教訓。** n=5 時 gpt-oss 中位數 7007ms、
gemma 10297ms，看起來 gpt-oss 快 32%；**放大到 n=15，兩者中位數變成 9217 vs 9209ms —— 相差
8 毫秒，實質相同**。那個 32% 幾乎全部是抽樣雜訊。gpt-oss 唯一真實的差異是**變異度大 3.6 倍**，
而在一個由 p90 與逾時定義的判準下，變異度大是純粹的壞處。
**教訓：延遲比較 MUST NOT 以 n=5 下結論**，尤其判準是 p90 時。

**第二段（`suggestion-kb`，帶 3 筆命中，prompt 879 字）** —— 受 003 FR-014 的 15 秒逾時約束：

| | `gemma-3-27b`（n=15） | `gpt-oss-20b`（n=5） |
|---|---|---|
| 中位數 | **10025ms** | 11877ms |
| 最快 | 9474ms | 6822ms |
| **最慢** | **13032ms** | 21204ms |
...
⚠️ **gemma 第二段的 n=5 同樣被 n=15 推翻**：n=5 時中位 11384ms、且有 1/5 次達 16891ms
**破了 FR-014 的 15 秒逾時**；n=15 下中位降為 10025ms、最慢 13032ms、**0/15 破線**。
當時據此寫下的「以 gemma 跑第二段約兩成機率整批失敗」是錯的。
⚠️ 另外 gemma 第二段的輸出**極度穩定**：15 次全部產出同樣的三張卡、引用同樣三筆 SOP。

三個結論：

1. **⛔ 情緒 agent 的 −48% 完全沒有移植過來。** gpt-oss 在情緒任務上的優勢是*範圍更窄*
   （4743～9210ms），但在建議卡任務上範圍**更寬**（n=15 第一段 5216～18130）。
   n=15 下**連中位數優勢都不存在**（差 8ms），只剩 3.6 倍的變異度 —— 而**逾時是被最慢值
   打死的**（本節前述原則）。第二段同樣沒有改善，中位數還略差。
2. **⛔ `gpt-oss` 的引用覆蓋會漏，而 `gemma` 不會。** gemma 第二段 5/5 次都完整引用
   kb-001／kb-002／kb-003，連卡片結構都幾乎一致；gpt-oss 只有 3/5：一次漏掉 kb-002
   （客戶明確問「這個月的費用可以退嗎」對應的費用減免 SOP），一次漏掉 kb-003 卻把
   kb-001 引用兩次。**兩者都沒有編造 id（憲法 4.3 皆通過），漏的是覆蓋而非正確性** ——
   但第二段存在的唯一理由就是提供引用，覆蓋不全等於它沒做到本份工作。
3. **FR-001 差一點點，而且差距是可能補得回來的。** 002 SC-001 是 **p90 判準**
   （「90% 以上的情況能在 10 秒內」），不是中位數判準。gemma 的第一段 p90 為 **10310ms
   —— 只超出 10 秒門檻 310ms（3.1%）**，10 秒內比例 73%（判準要 90%）。
   換模型補不了這個缺口（gpt-oss p90 10439ms 更差），但**還有一個從未量測的固定成本槓桿**：
   `callAgent()` 未傳 `user_id`，導致每次呼叫多一趟 `POST /ai-agent/chat-client/auth/user`
   往返（見本節下方）。那是**每次呼叫都付、且對 p90 與中位數同時有效**的固定成本，
   量級若在數百毫秒即足以翻轉判定。⚠️ **仍未量測，MUST 在 004 plan 拍板 FR-001 時限前量。**
   （→ 已量：54ms，補不上，見下方 `user_id` 小節。）
   **裁決（2026-08-29，004 clarify）：002 SC-001 的建議卡門檻由 10 秒改為 20 秒（p90）**，摘要／情緒維持 10 秒、
   3 秒骨架不變。理由：缺口是模型延遲，沒有任何 repo 內的槓桿補得回來，留著是一條明知達不到的門檻。
   > ⚠️ **「摘要／情緒維持 10 秒」這半句於 2026-09-01 被推翻**：001 SC-005 已拆為
   > **摘要 10 秒／情緒 15 秒**（情緒同時把批次改為並行）。3 秒骨架與建議卡的 20 秒不變。
   > 詳見本節下方「端到端量測：SC-005 的兩個區塊成本結構不同」與 §18 M2 驗收。
   20 秒對 p90 10.31 秒有近一倍餘裕，平台漂移 36% 後（≈14 秒）仍在內。⚠️ 與 001 FR-014 的 15 秒逾時耦合：
   逾時不動時 20 秒實質＝「首次呼叫逾時率 ≤10%」；逾時若提到 20 秒，退避預算 MUST 同步提到 ≥45 秒
   （`1+20+4+20`），該裁決依本節下方規定留到 004 plan。
...
| 知識庫檢索 | `nova-pro` | 11907ms | 20080ms ⚠️ | （逾時另計 30 秒） | 12/12 命中 ⚠️ |

⚠️ **知識庫那一列的兩個數字已於 2026-08-29 被 004 T032 的量測推翻**（n=10、真實對話）：
最慢由 20080ms 上修為 **22870ms**，「12/12 命中」的 30 秒涵蓋率實際是 **9/10**（出現一次逾時）。
本表其餘各列未重測，維持原值。詳見 §12「②-2 檢索本身需要 9～23 秒」一段的二次訂正。

⚠️ **摘要 agent 的 42.9 秒尖峰這 15 次沒有重現**（最慢 7646ms）。它曾是「FR-014 是否放寬」
的主要理由，該理由在 2026-08-29 時**沒有新證據支持**。但 n=15 也不足以排除罕見尖峰 ——
42.9 秒那次是真實發生過的，不是量測誤差，**MUST NOT 據此認定問題已消失**。

> ⚠️ **2026-09-01：尖峰重現了。** `npm run spike:verify-provider` 三次呼叫中，
> 第 2 次 `summarize()` 撞上 SDK 的 30 秒 HTTP 逾時（另兩次正常，narrative／topics 皆正確）。
> 上面那句「目前沒有新證據支持」自此**不再成立**，已改為限定在 2026-08-29 當時。
>
> ⚠️ **但這不構成放寬 FR-014 的 15 秒的理由**（理由見下方兩次量測後的結論）。
> 一個容易混淆的事實先講清楚：**spike 直接呼叫 provider，不經過 `withRetry()`**；
> 生產路徑的 `runColdStart()`／`runIncremental()` 一律包在 `withRetry()` 裡
> （`copilot-analysis.ts:502`）。所以「spike 整支失敗」證明不了「生產環境該次分析失敗」。
...
⚠️ **第一次那批是暫時性的降級時段，不是永久劣化** —— 重測就回到 11 秒等級。
但**也不是回到原狀**：中位數 11 秒是 8/29 基準的 1.75 倍，且 6 次裡有 4 次超過 10 秒。

⚠️ **這個「時段相關的不穩定」本身就是結論**，比任何單次的中位數都重要：
同一份輸入在同一天可以是 6 秒、11 秒或 52 秒，我方沒有任何方式預先知道現在是哪一種。
先前（2026-09-01 稍早，n=3）曾據此推測是「隨機雙峰、重試接得住」，**那個推測被 n=15 推翻**
——降級時段裡 11/15 破線，重試只是把三次失敗疊在一起，接不住。

**結論：FR-014 的 15 秒維持不動**，但理由與先前不同：
...
⚠️ **對 M2「10 秒 p90」驗收項的影響**：即使在恢復後的時段，6 次裡也有 4 次超過 10 秒。
該項在目前狀態下**不會通過**，且成因是模型延遲，repo 內沒有槓桿補得回來
（與 002 SC-001 當初把建議卡門檻從 10 秒改為 20 秒是同一種情況）。
⚠️ **這一節的三列數字會過期**，判斷「現在是哪一種時段」時 MUST 重跑量測，不要引用本表。

> ⚠️ **上面那句「repo 內沒有槓桿補得回來」只對摘要成立，對情緒是錯的**
> （2026-09-01 的端到端實測推翻，見下一節）。寫那句話時只量得到單一 agent 的延遲，
> 而 SC-005 的判準是**區塊**何時呈現 —— 摘要是 1 次呼叫，情緒卻是 N 批呼叫，
> 情緒那 N 有一個 repo 內的槓桿叫做並行度，當時完全不在視野內。
> **教訓：「沒有槓桿」是一個很容易說得太早的結論**，說之前先確認量的粒度
> 與判準的粒度是同一個。
...
先前所有延遲數字都是**單一 agent 的單次呼叫**（`spike:agent-latency`），而 001 SC-005 的判準是
**區塊的實質內容何時呈現**。兩者對摘要幾乎等價（1 次呼叫），對情緒完全不等價
（每 `SENTIMENT_CHUNK_SIZE` 則切一批）。這個落差正是該驗收項一直沒被驗掉的原因 ——
用單一呼叫的數字去推 SC-005 會**系統性低估**。`scripts/spike/21-progressive-citations.ts`
於本日補上兩者，證據存於 `scripts/spike/out/21-progressive-citations-*.json`。
...
⚠️ **摘要三輪皆未達且無槓桿**，使用者裁定門檻維持 10 秒並標記為已知落差（見 §18 M2）。
⚠️ **建議卡第一段三輪 p90 高度一致（28.7／28.8／29.3 秒）**，形狀是「首次破 15 秒逾時
→ 退避 1 秒 → 第二次成功」。**約三分之一的第一段呼叫撞上單次逾時**，這是 2026-08-29
「12/12 在 20 秒內」之後出現的新狀況，M2 該項已據此由 ✅ 改為未通過。
...
承載其單次逾時（`server/services/copilot-analysis.ts`）。
...
此處原本記載「兩段可掛不同 agent」為 004 的取捨選項，理由是「兩段的約束方向相反
（第一段要快、第二段要引用準），第一段掛快模型、第二段掛穩模型」。
**該理由建立在 n=5 的誤判上，已於同日被 n=15 推翻**：gemma 與 gpt-oss 的第一段中位數
只差 8ms，而 gemma 的 p90、σ、第二段引用覆蓋全部更好 —— **沒有「更快的模型」可以放到
第一段**，兩段的最佳選擇是同一顆，約束方向並不分歧，拆開也沒有東西可分別擺。

拆開唯一剩下的槓桿是「第一段用更短的 system prompt」。prompt 長度確實有效
（571 字對 879 字＝中位 9209 對 10025ms，約 2.6ms/字），但**第一段已經是短的那一份**
（`knowledgeHits: []`），要再縮只能動 agent 在 iMBrace 後台的 system prompt ——
那份 prompt **不在本 repo 裡**，改了無法版控、無法在 CI 重現、事後也無法歸因。
代價則是多一個「被人靜默改掉模型」的失效面，而這不是假設性風險：知識庫 agent 的模型
就曾被改掉且無任何紀錄（見本節模型表的警告），害一組實測數據作廢。兩份 prompt 各自漂移
還會讓兩段的卡片語氣不一致 —— 而 004 要求第二段**整批替換**第一段，不一致客服會直接看見。
...
**2026-08-29 實測**（`npm run spike:userid`，隔離量測該趟呼叫 20 次）：

| | |
|---|---|
| 中位數 | **54ms** |
| p90／最慢 | 64ms／572ms（第一次，冷連線） |
| σ | 114ms |
| id 穩定性 | **20/20 皆同一個 id** ✅ 可快取 |
| 傳入 `user_id` 後仍正常輸出 | **5/5** ✅ 不影響結果 |

⛔ **它補不上 FR-001 的缺口。** 建議卡第一段 p90 超出（當時的 10 秒）門檻 310ms，而這只值 54ms（17%）。
（門檻已於同日改為 20 秒，缺口不再存在；但本段結論——它是衛生問題不是效能解方——不變。）
先前把它列為「可能翻轉 FR-001 判定」的槓桿是**推測錯誤**，已於此更正。
✅ **但仍應該做**：省一趟往返、id 實測可快取、傳入不破壞輸出，且 `streamChat()` 的型別
本來就宣告了 `user_id`（不需繞道）。它是**衛生問題，不是效能解方**。
...
> ⚠️ **後三列是 2026-08-28 新增的（`specs/003-analysis-trigger-policy`）**，修正 2026-08-27 於真實環境
> （iMBrace 平台中斷期間）發現的無限重試缺陷：面板每 20 秒重跑一次完整分析、每輪內部各自重試 2 次、
> 客服按下離開後仍不停止，實測換算約 **3,780 次 AI 呼叫／小時／對話**。
>
> **三處都是判斷寫在錯的那一層**，不是缺少機制：
> ① `createWatchRegistry.watch()` 對心跳與真實變化走同一條路（而 `attach()` 帶有送快照＋補跑的副作用）；
> ② 分析失敗時 `sentimentBlock.timeline` 與 `summaryBlock.summary.basedOnMessageId` 都不推進，
>    同一批訊息因此永遠被判定為「尚未涵蓋」；
> ③ `runIncremental()` 的門檻寫的是「分析狀態存不存在」，而分析狀態有 2 小時 sliding TTL、LEAVE 不會清掉它。
>
...
> ⚠️ **2026-08-27（憲法 v3.0.0）**：本節原本寫的是「背景對話僅輕量情緒分類，不生成建議卡、不查知識庫」，
> 成本自然收斂到「同時只有一個對話在跑完整 AI」。該設計在真實情境下不成立 —— 客服 JOIN 對話 A 後
> 切去回應對話 B，A 的客戶通常仍在發言，切回時客服面對的是一批過時建議卡且要從頭再等 5～12 秒。
> 現行做法保留分級機制，但背景分級的內容從「不跑」改為「跑受限子集 + 節流」。
...
>
> ✅ **2026-09-01 實測：後台 prompt 已經在回這兩個欄位**（`npm run spike:verify-provider`，
> `out/16-provider-runs.json`）。三次呼叫中兩次成功、兩次都帶齊 `narrative` 與 `topics`
> （實例：`topics=["網路斷線","數據機"]`，長度與張數都在 `SUMMARY_TOPIC_MAX_*` 的上限內）；
> 唯一缺席的那次是**整支 `summarize()` 撞上 30 秒逾時**，不是欄位缺席。
> 即「`summarize()` 成功時 2/2 帶齊」。因此目前**不需要去後台補 prompt**。
> ⚠️ 這個結論的保存期限取決於 repo 外的設定，**不會有任何東西通知我們它變了** ——
> 動到摘要欄位、或懷疑正文變短時，重跑上面那支 spike（它會分別計數 `narrative`／`topics`，
> 不與 `summaryOk` 合併，正是為了讓「欄位安靜消失」現形）。
>
...
三項限制。**2026-09-01 使用者裁定拆開處置**，因為三者的性質根本不同，包成一包會讓
已經有結論的那一項永遠看起來像沒做完：
...
**② 情緒分析批次大小是機率賭注，非保證值 —— ✅ 已接受的取捨，不是待辦**（2026-09-01 澄清）。
真實對話 16 則客戶發言，單次呼叫（不分批）實測延遲 12.7～29.9 秒，遠超 FR-014 的 15 秒單次
逾時。已改為每批固定 6 則（見 `server/services/copilot-analysis.ts` 的 `SENTIMENT_CHUNK_SIZE`），
但延遲不是單純隨則數線性增加、平台本身有明顯波動——4 則批次 3 次都在 8.5～9.7 秒，
6 則批次量到 10.0～**18.6 秒**（超過 15 秒門檻），8 則批次曾連續三次嘗試全部逾時。
沒有一個批次大小能保證「絕對不逾時」；**使用者已決定接受「偶爾需要客服手動重試」這個下限，
6 是取捨後的中間值。** 本項先前被上一版的「留待後續任務評估」一起蓋住，
造成一個已拍板的決定看起來像未完成工項——那是文件內部不同步，不是待辦。

> ⚠️ 原文還有一句「批次切得越小，單次逾時機率越低，但長對話需要的批次數越多、**總耗時越長**」。
> **後半段自 2026-09-01 起不再成立**：批次已改為有上限的並行（`SENTIMENT_CONCURRENCY`），
> 總耗時不再隨批次數線性增加，而是隨 ⌈批次數 ÷ 並行度⌉。前半段（批次越小、單次逾時機率越低）
> 不變，因此「6」這個取捨的依據仍然成立 —— 但**再談批次大小時 MUST 用新的成本模型**，
> 不要沿用「切小＝總時間變長」這個已經失效的直覺。
...
> ⚠️ **2026-08-28 訂正：伺服器重啟的後果與上一句相反，而且更嚴重。**
> 本段原本寫「或伺服器重啟過……也會觸發全新的冷啟動分析」——**實際上不會觸發任何分析**。
> `runColdStart()` 唯一的呼叫點是 `join.post.ts`，而平台側的 JOIN 在重啟後依然存在，
> 客服看到的是「已接手」的畫面，不會再按一次接手，於是冷啟動永遠不會發生，
> 面板永久停在空狀態且完全不報錯。症狀不是「浪費 AI 呼叫」，是「功能靜默消失」，
> 處置方向也因此不同。當晚真實環境重現四次，已列為 M2 驗收的未完成工項。

**①③ 的立案內容**（2026-09-01 起為「M3 開工前決策」的一部分，與憲法 5.3 的待修憲並列）：
...
⚠️ **這是「①③ 合併」，不是原本的「三項合併」** —— ② 已於 2026-09-01 澄清為已接受的取捨，
不在這個立案範圍內（見上方 ②）。**MUST NOT 因為它們同處一節就把 ② 一起拉進來重議。**
...
（2026-08-27 對真實知識庫實測；延遲數字已於 2026-08-29 加大樣本後訂正，見下）。兩件事分開講：
...
*要多久*：2026-08-29 對現行的 `us.amazon.nova-pro-v1:0` 單獨取樣 12 次
（`npm run spike:knowledge-latency`）：**最快 9370ms、中位 11907ms、p90 16929ms、最慢 20080ms**。
⚠️ **原記「最快 13.0 秒、沒有任何一次低於 13 秒」已被推翻**（新取樣 12 次中有 7 次低於 13 秒）——
原數字是三個模型混合、每個只取樣三四次，其中 nova-pro 僅 4 個點。差異最可能來自**原樣本太小**
而非平台變快（同日在建議卡 agent 上已連續兩次發生 n=5 被 n=15 推翻，見 §8.2b），查詢內容亦不同。
同組織不掛知識庫的 agent 單純回一句話只要 2.6～3.8 秒，所以慢的是檢索而非推論。
**兩個操作性結論不變且證據更強**：8 秒 **0/12** 命中、30 秒 **12/12** 命中。

⚠️ **2026-08-29 二次訂正（004 T032 的真實環境量測，`npm run spike:progressive`）**：
上述區間與「30 秒 12/12」兩者都已被更多樣本推翻，**幅度不大但方向明確**——
n=10（4 段真實對話、4 輪）：成功 9 次落在 **13.9～22.9 秒**（中位 18.4 秒），
**另有 1 次觸發 30 秒逾時**。也就是說：
  - 最慢值由 20080ms 上修為 **22870ms**（本節上方的 9.4～20.1 秒區間是 n=12 的結果，**不是上界**）
  - **30 秒的涵蓋率是 9/10，不是 12/12** —— `KNOWLEDGE_SEARCH_TIMEOUT_MS` 會真的觸發，
    這不再是理論上的可能性
逾時當下的降級行為經實測**完全正確**：建議卡序列仍是 `analyzing → ready/pending → ready/none`，
`status` 維持 `ready`、第一段的卡照常可用，沒有轉 error（004 FR-003／002 FR-004）。
⚠️ 因此 002 SC-002b 的「100% 在 35 秒內落定」在現行平台**已有反例**（逾時那次落定於 30.0 秒
仍在 35 秒內，但那是因為逾時上限恰好擋住了它——若上限放寬，那一次會超出）。

⚠️ **2026-08-29（004 落地）：快查與建議卡改用同一個逾時值 `KNOWLEDGE_SEARCH_TIMEOUT_MS = 30_000`。**
本段原本主張「兩者必須用兩個不同的值」，理由是建議卡走「先檢索再生成」的**串行**流程、
受 SC-001 門檻約束，因此另立 8 秒短逾時。上面那組數字證明的其實是：**8 秒在生產路徑上
100% 逾時**，那個短逾時的實際效果是「建議卡永遠拿不到引用」，而不是保護門檻。

004 把建議卡改成**兩段式**（先出無引用版本並標示「尚未引用知識庫」，檢索回來有命中時
以命中結果重新生成整批並自動換上），檢索因此不再擋在生成前面，30 秒可以承受，
短逾時的存在理由隨之消失、常數已刪除，並由 `test/contract-guards.test.ts` 的守衛防止復活
（連 `= KNOWLEDGE_SEARCH_TIMEOUT_MS` 的別名也不行 —— 別名日後照樣會被改成別的數字）。
規格見 `specs/004-progressive-citations/`。延遲本身仍列在 `docs/IMBRACE_QUESTIONS.md` 0-3h。
...
> ⚠️ **「AI 分析失敗」那一列到 2026-08-28 為止都只有前半是真的。** 承諾是「重試用盡後顯示
> 『暫時無法分析 [重試]』」，而實際上那個錯誤狀態每 20 秒就被整輪重跑一次覆蓋掉 ——
> 客服看到的是永遠在「重試中」跳動的區塊，手動重試按鈕多數時候根本按不到。
> `specs/003-analysis-trigger-policy` 修正之後，實作才真正符合這一列與憲法 3.2 的敘述：
> **那一輪重試預算（001 FR-014，最多 2 次、1s→4s、總預算 40 秒）一字未改**，
> 改的是「用盡之後」—— 同一批訊息在同一區塊不再被自動重試，狀態就停在 error 等人按。
> 觸發策略的完整表格見 §11.1。
...
- [x] JOIN 後 **15 秒**內**情緒**的實質內容呈現（90 百分位）—— 2026-09-01 實測後訂定並通過
      實測 p90 **12.7 秒**、最慢 14.4 秒、中位 7.7 秒（`spike:progressive -- --repeat 3`，n=15）。
      ⚠️ **門檻由 10 秒改為 15 秒是 2026-09-01 的使用者裁定，改的同時也改了程式**——
      不是單純放寬。情緒的總時間 **≈ ⌈批次數 ÷ 並行度⌉ × 單次延遲**，先前批次是依序送出，
      於是 10 秒門檻**只在客戶發言 ≤ 6 則時成立**（實測 1 批 6/6 過、3 批 0/6、5 批 0/3）。
      改為有上限的並行（`SENTIMENT_CONCURRENCY = 3`）後中位由 16.9 秒降到 7.7 秒。
      ⚠️ **並行化的風險已一併實測且沒有發生**：單次呼叫 n=39、中位 6.6 秒、最慢 11.7 秒、
      **破 15 秒逾時 0 次、失敗 0 次**（依序版由總時間反推的每批中位是 5.5～6.4 秒）。
      調高並行度前 MUST 重跑並同時看這兩列，理由見 `copilot-analysis.ts` 的常數說明
      ⚠️ 15 秒**不是對所有長度成立**：50 則客戶發言（9 批＝3 波）約 20 秒，會破。
      這個門檻買到的是「常見長度有明確承諾」，不是全稱保證
- [ ] JOIN 後 10 秒內**摘要**的實質內容呈現（90 百分位）
      ⚠️ **已驗證，且不通過** —— 2026-09-01 三輪實測（各 n≈15，同一組 6 段真實對話）：
      中位 **7.6／10.8／10.0 秒**，10 秒內只有 **8/15、5/14、6/14**（判準要 90%）。
      **三輪皆未達，因此不是單次的降級時段。**
      ⚠️ 成因是模型延遲的尾巴，**repo 內沒有槓桿補得回來**（與情緒相反 —— 情緒有並行度
      這個槓桿，摘要是單次呼叫，沒有）：同一份輸入同一天量到 7.5、11.7、45.3 秒。
      ⚠️ **使用者 2026-09-01 裁定門檻維持 10 秒、不放寬**，理由是 SC-005 不擋任何執行路徑
      （會擋的是 FR-014 的 15 秒單次逾時），放寬只是把判準改成會通過；而這條門檻守的正是
      001 的價值主張「客服不必重讀完整歷史」，摘要 25 秒才出現時客服早已自己開始讀了。
      ⚠️ 本次**只動判準、未動 FR-014 的任何數字**。日後若為了讓摘要落進門檻而調高那 15 秒，
      §8.2b 的連動條款（逾時提到 20 秒則退避預算 MUST 同步提到 ≥45 秒）就會生效
- [ ] JOIN 後 **20 秒**內首批建議卡呈現（90 百分位）
      ⚠️ **2026-09-01 由 ✅ 改為未通過** —— 原證據是 2026-08-29 的 n=12：p90 10.5 秒、
      12/12 在 20 秒內（004 T032）。同一支腳本 2026-09-01 連跑三輪，p90 為
      **28.7／28.8／29.3 秒**，20 秒內的比例 10/12、6/7、6/7，**三輪皆未達**。
      ⚠️ **三次獨立量測給出幾乎相同的 p90，這個一致性本身就是結論**：那不是抖動，是
      「首次呼叫破 FR-014 的 15 秒逾時 → 退避 1 秒 → 第二次成功」這個固定形狀
      （15 ＋ 1 ＋ 13 ≈ 29 秒）。也就是說**約三分之一的第一段呼叫撞上了單次逾時**。
      ⚠️ **MUST NOT 據此把門檻調高到 30 秒** —— 20 秒是 2026-08-29 從 10 秒放寬時
      經過裁決的數字，當時的餘裕論證仍然有效；現在變的是首次逾時率，那是另一個問題。
      ⚠️ 門檻是 20 秒不是 10 秒（建議卡 agent 單次生成 p90 就有 10.31 秒），**不要調回去**。
      ⚠️ 此處量的是**第一批可用的卡**；帶 SOP 來源的版本由 004 第二段換上，第二段落定三輪
      p90 為 32.8／34.9／43.8 秒（上限 50 秒，**通過**），兩者是不同門檻
...
**M2「3 秒」的語意（§18 M2）**：原驗收寫「JOIN 後 3 秒內出現摘要與首批建議」，讀起來像是要求 3 秒內產出實質內容——但 §6.2 的實測是 AI 單次呼叫中位數 5.0 秒、最慢 12.2 秒，那個門檻九成達不到。經 `specs/001-sentiment-panel` 的 clarify 收斂為兩條：3 秒衡量「面板已出現並標示分析中」（客服知道系統開始為他工作），實質內容另訂 10 秒 / 90 百分位，且允許逐欄漸進填入。連帶：切回已 JOIN 的對話時必須先顯示上次保留的結果而非重新 loading（§11.2 原寫「1–2 秒 loading 完全可接受」已修正）。⚠️ **2026-08-29 訂正**：上句的「10 秒」自此**只涵蓋摘要與情緒**；建議卡的實質內容門檻已由 10 秒改為 **20 秒**（002 SC-001，裁決見 `specs/004-progressive-citations/spec.md` Clarifications 2026-08-29 —— 建議卡 agent 單次生成 p90 10.31 秒，10 秒在現行平台必然驗收不過），§18 M2 的驗收項已據此拆為兩行。3 秒的骨架門檻三者一律不變。
