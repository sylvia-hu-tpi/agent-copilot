...
換模型後同一對話的全歷史冷啟動（多個 6 則批次依序跑）一次通過、零失敗。
...
- [ ] JOIN 後 10 秒內**摘要與情緒**的實質內容呈現（90 百分位，可逐欄漸進填入）
      ...
      ⚠️ **2026-09-01 出現第一個負向訊號**：`spike:verify-provider` 三次呼叫中有一次
      `summarize()` 超過 30 秒（另兩次正常）。n=3 不足以下結論，但摘要 agent 的尖峰
      （曾達 42.9 秒）確認**沒有消失**——量這一項時務必看尾巴而非中位數，詳見 §8.2 的延遲基準表
