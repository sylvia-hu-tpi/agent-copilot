...
**為何 `sender.type` 是必要的**：撞單防護需區分同事回覆與 AI 自動回覆（§10.4）；對話分段的分界為 `mode` 切換為 `manual` 的時刻；情緒分析只在 `sender.type === 'customer'` 的訊息上產生情緒點；客服自己送出的訊息不觸發重新分析（§11.1）。
...
依 `demo_agentCopilot02.png`，右欄自上而下共五個區塊：① 客戶情緒提示（處理中最常看）② AI 語意即時建議（處理中最常用）③ 知識庫自然語言快查（隨時可能用）④ AI 階段完整對話紀錄（可折疊，偶爾回顧）⑤ 結案摘要自動填入（只在結案時使用）。
...
訊息流使用虛擬滾動（`useVirtualList`）；建議卡數量上限 3–5 張，超過需捲動；情緒序列僅保留最近 N 點（建議 50）。
...
| AI 分析失敗 | 該區塊顯示「暫時無法分析 [重試]」，其他區塊照常運作 | ❌ 否 |
