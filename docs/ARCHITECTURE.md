...
/** 結案摘要（客服按下「結案」觸發，經人審後寫入 Data Board；⚠️ 2026-09-03 起不再由對話狀態變更觸發，見 §13.4 ②） */
...
>
> ⚠️ **2026-09-03：結案不變更平台上的對話狀態。** 本節原本寫的是「`updateStatus() → resolved`」——既把它當觸發條件，也當結案的最後一步。兩者都撤銷：
> 觸發條件**只有**客服按下「結案」；寫入 Data Board 成功後的最後一步**只是 LEAVE**（與「離開對話」相同的呼叫），對話在平台上的狀態維持原樣。
...
- [x] **UI 上已經有一行文案在對客服承諾這個尚未實作的行為** —— ✅ **2026-09-04 落差已消除**：
      ...
      因此 `conversation.exitHint` 的「結案＝產生摘要供確認後寫入」與實際行為一致
      （`specs/006` T033 已逐句對照，並刪掉語意已不存在的死鍵 `conversation.closeFailed`）。
      ⚠️ 下方原文保留為歷史紀錄，說明這個落差當初是**刻意**的。
      ✅ **連帶的 003 SC-007 已於 2026-09-07 補驗通過（3/3）** —— 3 位未參與者在按下之前
      皆說得出哪一個會留下紀錄，由 `specs/006` T052 承擔並執行。⚠️ 它曾於 2026-09-04
      被決定放行（訪談未執行、改與 PM 確認），**那個狀態已由本次實測取代**；
      SC-007 至此是「驗證通過」而非「結案而非驗證」。
      ⚠️ 受測者代號與逐字回答未留存（使用者回報結果），要複驗時素材與問句備妥於
      `specs/006-closure-handoff-summary/quickstart.md` §2 SC-005。
      ...
      **結案＝產生摘要供確認後寫入**」，而 M2 的「結案」當時只等同「離開 ＋ 停止分析 ＋
      隱藏面板」（`useConversationView.ts` 的 `closeConversation()`）。
      ⚠️ **這段描述的是 2026-09-04 之前的行為，已由 006 改掉** —— 保留是為了記錄
      「文案先行」這個決策當初的代價，不是描述現況。
      **2026-08-28 使用者決策：文案先行、M3 補上行為**，不改成描述現況——改了 M3 又要改
      ...
      —— ⚠️ **這個「要重新提出」已於 2026-09-07 執行完畢並通過**，見本條上方，此處為歷史原文
- [x] **同一份草稿重複寫入只留一筆；同一對話的多次結案各自並存** —— ✅ 2026-09-04 交付並驗證
      （`test/closure-idempotency.test.ts`：同一 draftId 重試 10 次恰好 1 筆、兩份草稿 2 筆並存、
      `q` 命中他人紀錄時仍走 createItem）。措辭已確認為訂正後的版本。 —— ⚠️ **本條於 2026-09-03 隨憲法 v4.0.0 訂正**，原文寫的是「重複觸發摘要為覆蓋而非新增」，那個方向是錯的：覆蓋會在「同一客戶被服務過多次」時銷毀服務歷史，也會在「同一次服務多位客服各自結案」時洗掉同事的工作成果。冪等的單位改為**摘要草稿**（憲法 5.3、§13.4 ③）
- [x] **按下「結案」產生結案摘要** —— ✅ 2026-09-04 由 `specs/006-closure-handoff-summary` 交付。⚠️ 原文為「resolved 產生結案摘要」，2026-09-03 訂正：對話狀態變更**不是**觸發條件，結案也**不變更**平台對話狀態（§13.4 ②）
...
- [x] **結案摘要的涵蓋區間由客服選定，且區間與則數隨紀錄寫入** —— ✅ 2026-09-04 交付
      （`ClosureScopePicker.vue` ＋ `period_start`／`period_message_count`／`period_origin` 三欄）。 —— 2026-09-03 新增（§13.4 ④）。⚠️ 三種自動推導方式都會出錯，該節已列反例，不要重新提案
...
> ⚠️ **上列五個結案相關的勾記的是「2026-09-04 交付」，但它們的手動驗收要到 2026-09-07 才成立** ——
> 第一輪走查是在有六個缺陷的畫面上進行的（`specs/006-closure-handoff-summary/quickstart.md` §6.8），
> 修完後重跑，T053（閒置 12 分鐘無寫入）與 T054（§3 走查步驟 1～8c ＋中途三件事）於 2026-09-07 第二輪通過，
> 紀錄見該檔 §6.4.2。✅ **SC-005（＝ 003 SC-007，3 位未參與者讀得懂兩個出口）已於 2026-09-07 通過（3/3）** ——
> 曾於 2026-09-04 被決定放行（未執行），該狀態已由實測取代；受測者代號與逐字回答未留存。
> 綠燈基線 2026-09-07 重跑：`typecheck` ✅、`vitest` 58 檔 726 項 ✅、`build` ✅、`smoke` ✅。
>
