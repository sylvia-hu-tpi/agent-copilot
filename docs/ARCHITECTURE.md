...
#### ⚠️ 摘要的落差發生在管線內，不在 agent 本身（2026-09-02）

同一天、相隔約一小時的兩種量法：

| 量法 | n | 中位 | 最慢 | 10 秒內 |
|---|---|---|---|---|
| 隔離單次（`spike:agent-latency -- summary 20`） | 20 | 5.2 秒 | **11.7 秒** | **19/20 ＝ 95%** |
| 端到端（三輪合併，冷啟動全區塊同時跑） | 45 | 6.6 秒 | 45.7 秒 | **33/45 ＝ 73%** |

**隔離時最慢只有 11.7 秒，端到端卻有 4 個樣本落在 22.9／24.2／29.1／45.7 秒** ——
那是「撞 FR-014 的 15 秒逾時 → 退避 1 秒 → 第二次成功」的形狀（15＋1＋X），
也就是**在管線裡有摘要呼叫超過 15 秒，而隔離時一次都沒有**。
另外 5 個未達的樣本落在 10～14.4 秒，是單次慢但沒撞逾時。

⚠️ **這代表落差來自管線內的競用，不是摘要 agent 慢。** 冷啟動時摘要、建議卡第一段、
以及 `SENTIMENT_CONCURRENCY = 3` 的三批情緒同時在飛，四到五個呼叫打同一個平台租戶。

⚠️ **未證實，且方向可能不只一個。** 2026-09-01 為了救情緒而加的並行度，有可能把成本
轉嫁給了摘要（該日第 3 輪〔並行 3〕的摘要 p90 是三輪最差的 49.8 秒）；但也可能只是
平台自身的尾巴。**乾淨的對照是把 `SENTIMENT_CONCURRENCY` 暫時設回 1 跑一輪**，
比較摘要的分佈 —— 在做這個對照之前 MUST NOT 宣稱兩者有因果。

⚠️ **一併記下**：這也是「隔離單次量測 MUST NOT 用來預測驗收」的第二個實例。
95% 與 73% 的差距全部產生在管線裡，而隔離量測看不到管線。
...
同一組六個對話、同一份程式、同一組門檻，三輪（本機 10:29／10:58／11:56，各 n=15）：
...
- [ ] JOIN 後 **15 秒**內**情緒**的實質內容呈現（90 百分位）—— ⚠️ **2026-09-02 由通過退回未定**
      ...
      ⚠️ **依批次數拆開後成因很明確**：1 批 9/9、2 批 9/9 全過；**3 批 12/17、5 批 5/9 破**。
...
- [ ] JOIN 後 10 秒內**摘要**的實質內容呈現（90 百分位）—— **未通過，且成因已與原記載不同**（§8.2b）
      ...
      ⚠️ **落差發生在管線內，不在 agent 本身**：隔離單次量測是 19/20 ＝ 95%、最慢 11.7 秒，
      而端到端有 4 個樣本呈「撞 15 秒逾時 → 退避 → 重試」的形狀（22.9～45.7 秒）。
      也就是**管線裡有摘要呼叫超過 15 秒，隔離時一次都沒有**（§8.2b）。
      這使得「repo 內沒有槓桿」這句話**需要重新檢查** —— 若競用成立，減少同時在飛的呼叫
      就是一個槓桿。⚠️ 但因果未證實，乾淨的對照是把 `SENTIMENT_CONCURRENCY` 暫設回 1 跑一輪。
