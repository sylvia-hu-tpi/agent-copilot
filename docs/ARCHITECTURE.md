...
⚠️ **兩段可以掛不同 agent。** 兩段的約束方向相反（第一段要快、第二段要引用準），而
`ImbraceAgentProvider` 的 agent id 本來就由環境變數注入。若 004 要同時滿足兩者，
「第一段用快模型的 agent、第二段用穩模型的 agent」是可行的設計選項，代價是後台多一個
agent 與多一組 env。**列為 004 plan 的取捨項，本節不預設結論。**
