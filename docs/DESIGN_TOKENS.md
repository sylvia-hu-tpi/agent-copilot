...
> 需要看畫面時開①。
>
> **凍結時間點：2026-09-08 17:24 版畫布**。七個 artboard（1a／1b／1c／1d／2a／**2b**／3a）全部是逐字擷取的規格。
...
>
> ⚠️ **2026-09-08 這一輪把整條頂列重新擷取了**（§8.1 新增「頂列」一節）。
> 觸發點是畫布補上了**「淺／深色主題切換鈕」**（`toggleTheme`）—— 那是**第一顆讓使用者
> 自己換主題的入口**，在此之前深色只是畫布上並排的另一種塗色，產品裡沒有任何地方切得過去。
> ✅ **實作已於同日補上**（`app/layouts/console.vue` ＋ `nuxt.config.ts` 的 `colorMode`）；
> 畫布沒回答的兩題由使用者裁示：**記住上次的選擇**、**首次進入固定 light**（不跟隨系統）。
> 同一天 Design 另外兩度主動對齊我方：分隔線兩側各 10px（`margin-left:-2px` 的做法），
> 以及補上 `1d-empty` 漏畫的主題鈕與頭像（見 §9）。
>
...
**目前是空的。** 2026-09-08 那一輪核出的落差（沒有主題切換入口、頂列頭像 26→28px、
分隔線兩側 10／10、左欄分組標題 13→12.5px）**當天全部補上**，
...
**主題切換鈕**（⚠️ 2026-09-08 新增，`toggleTheme`）：
...
>
> ⚠️ **哪些畫面有這顆鈕**：1c 與 **`1d-empty`**（已登入、只是還沒選對話）有；
> **`1d-loading` 沒有**（骨架階段，右側是 `--skel` 佔位條）；1a／1b 也沒有（尚未登入）。
> ⚠️ 判準是**「登入了沒有」，不是「有沒有選對話」** —— 2026-09-08 17:24 版之前
> `1d-empty` 漏畫了整組，我方回報後 Design 補上。
>
...
| 頂列 | 「台灣客服中心」／「已連線」（⚠️ 與 1c 逐字相同；畫布已無「· 即時同步」）。⚠️ **兩個狀態的頂列右側不一樣，不要抄同一份**：`1d-empty` 與 1c **完全相同**（連線 pill ＋ 主題切換鈕 ＋ 分隔線 ＋ 28px 頭像，見 §8.1）—— 那裡已經登入、只是還沒選對話；`1d-loading` 則是骨架，組織名與右側各一條 `--skel` 佔位條（`height:10px`／`width:120px`／`90px`／`radius:4px`），**沒有主題鈕也沒有頭像**。⚠️ `1d-empty` 這一組是 2026-09-08 17:24 版才補上的（先前只有裸的圓點＋文字），舊版畫布的樣子不要當規格 |
...
>
> ⚠️ **`dc-import` 的元件內容就在 artifact 裡**（`CopilotPanel.dc.html`：2026-09-08 17:24 版解開後 **104 KB**，第 6 區塊與涵蓋範圍選擇器都在裡面，不在 `page.html`；`__bundler/template` 本身解開後約 **473 KB**，其中絕大部分是內嵌的 `@font-face`，先把 `@font-face { … }` 整批濾掉再讀，剩下約 143 KB 才是版面），
