...
3. **驗證碼是 6 格分離輸入，不是單一輸入框**（確定答案）：6 個獨立 `<input>`，各 `56×56px`，`text-align:center`，`font-size:22px`，`font-family:'IBM Plex Mono'`，`maxlength=1`，`border-radius:9px`；focus 時 `border-color:var(--navy-2)` + bg 轉 `var(--surface)`
   ...
   > 連同 §6 的 OTP 有效期與「左上角」，這是本文件**三處**「畫布錯、實作對」的落差之一——
...
- 「輸入驗證碼」／「已寄送至 {{maskedEmail}}，10 分鐘內有效。」
  ...
  > 這是本文件**三處**「畫布錯、實作對」的落差之一，另兩處是 §4.2 的 OTP 字元集與本節的「右上角」。
- 按鈕「驗證並登入」
...
- 「你隸屬於 {{n}} 個組織。選擇要進入的組織，之後可從**左**上角切換。」
  > ⚠️ **畫布原文寫的是「右上角」，那是錯的，不要照抄。**
  > 畫布**自己的 1c 頂列**把組織名放在左邊（`AGENTCOPILOT` 徽章之後，見 §8.3），
  > 是畫布內部矛盾。文案要描述使用者真的看得到的位置，因此實作寫「左上角」
  > （2026-08-29 由使用者指出）。這是本文件**第三處**「畫布錯、實作對」的落差。
  >
  > ⚠️ 另有一個尚未解決的問題：這句話承諾的「可以切換」目前**沒有實作** ——
  > 重新 exchange 需要 `loginToken`，而它只存在於 `PendingOrgSession`，
  > 選完組織後就沒有了。詳見 `docs/M2-UI-PARITY.md` 的 U-3。
