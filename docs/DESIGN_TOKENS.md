...
3. **知識庫自然語言快查** —— 搜尋輸入框（placeholder 為示範查詢句，如「發票補寄要多久」）＋ 結果清單，每筆：標題 ＋「插入為回覆」／「展開全文」按鈕；過期文件會多一條警示列（如「⏱ 已超過 12 個月未更新，引用前請確認」，疑似沿用 `--warn` 系色票）。⚠️ **本節截圖尚未同步修正**：目前 `docs/wireframe/05-copilot-panel_4status_01.png` 此區塊仍畫有「SOP #12 · 2026/05」一類的編號＋日期格式，與第 2 區塊已移除編號的決定不一致；已與畫布擁有者確認**此區塊的編號也應一併移除**（僅顯示標題＋更新日期），截圖與 artifact 待後續一併更新，本文件先依最終決定描述，不逐字照抄目前截圖上這一處
