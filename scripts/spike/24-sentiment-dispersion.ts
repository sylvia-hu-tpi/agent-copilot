 ...
 *
 *     ⚠️⚠️ **但因果只成立這一個方向，讀反過已經害人白忙一場（2026-09-02）。**
 *     這個數字變大**不足以反推**分數帶失效 —— system prompt 的另一條規則明文要求
 *     「判斷某一則時必須參考同批的前後文」（那是為了解決「好，那我再等等」被判成
 *     85／calm 的假尖峰），而 B 段量的正是上下文敏感度。只要那條規則有效，
 *     **落在分數帶界線上的句子，B 段就不可能是 0**：本樣本的「我要申訴，順便問一下
 *     解約要怎麼辦」同時命中兩級的關鍵詞（「要求投訴或求償」→ angry、
 *     「威脅離開」→ frustrated），光是上下文長短就能讓它在兩級之間移動 25 分，
 *     而兩個判讀都在 prompt 的字面定義內。
 *     數字變大時**先跑 `npm run spike:agent-prompts`**（1 秒）—— 它把後台 prompt 與
 *     `docs/AGENT_PROMPTS.md` 的快照逐字元比對，那是直接證據；再跑一輪本 probe
 *     只是換一組間接證據。經過見 ARCHITECTURE 附錄 C-3。
