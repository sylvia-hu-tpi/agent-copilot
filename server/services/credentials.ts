 ...
 *
 * ── 這與憲法 1.3「不建立全域 SDK 單例」的關係 ─────────────────────
 * 憲法禁止的是**共用一個 client 去做所有人的操作**，理由是
 * 「JOIN 與訊息送出的歸屬會錯亂，稽核軌跡失去意義」——
 * 也就是說，這條約束的標的是**寫入操作的歸屬**。
 *
 * 因此本模組刻意設下兩道界線，讓借用不會踩到那個理由：
