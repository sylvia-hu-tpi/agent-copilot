 ...
 *
 * ⚠️ **2026-08-29 訂正：「沒有任何一次低於 13 秒」已被推翻。** 對現行的
 * `us.amazon.nova-pro-v1:0` 單獨取樣 12 次（`npm run spike:knowledge-latency`）：
 *
 *   最快 9370ms／中位 11907ms／p90 16929ms／最慢 20080ms，**12 次中有 7 次低於 13 秒**
 *
 * 原數字是三個模型混合、每個只取樣三四次得出的，其中 nova-pro 只有 4 個點（13.1～23.8 秒）。
 * 差異最可能的來源是**原樣本太小**（n=4 對 p90 沒有解析度），而非平台變快——同一天在
 * 建議卡 agent 上已經連續兩次發生「n=5 的結論被 n=15 推翻」（見 `docs/ARCHITECTURE.md` §8.2b）。
 * 查詢內容也不同，無法排除其影響。**現行區間應以 9.4～20.1 秒理解。**
 *
 * **但兩個操作性結論都沒有改變，而且證據更強了**：
 *   - 8 秒在生產路徑上 **100% 逾時**（新取樣 0/12 落在 8 秒內），快查恆顯示「暫時無法使用」
 *   - 30 秒足夠（新取樣 12/12 落在 30 秒內）
 *
 * ⚠️ **2026-08-29：SC-002 已拆為兩條**（002 spec）。原本的單一 25 秒門檻源自 n=9 的最慢值
 * 24.9 秒，而那筆出自上面已排除的 `qwen.qwen3-vl-235b-a22b`；且 25 秒比本常數的 30 秒**短**，
 * 使 SC-002 自己那句「逾時上限 MUST 短於此門檻」從未成立。現行為：
 *   - **SC-002a**：90% 在 20 秒內看到結果（貼著 p90 16.9 秒，18% 餘裕，退步驗收抓得出來）
 *   - **SC-002b**：100% 在 35 秒內落定 ＝ 本常數 30 秒 ＋ 5 秒 BFF 往返與渲染。
 *     **本常數 MUST 短於 35 秒**——SC-002b 的保障就是靠它，改動本常數 MUST 一併重看該門檻。
 *
 * 這不是選錯模型 —— iMBrace 的知識庫檢索延遲就是這個量級（已列入
 * `docs/IMBRACE_QUESTIONS.md` 詢問是否可調校）。
 *
 * ⚠️ **2026-08-29 起，建議卡路徑（004 FR-003）與快查共用此值**——原本為建議卡另立的
 * 8 秒短逾時常數已刪除。它存在的理由是保護「先檢索再生成」那條串行路徑的門檻，
 * 而 004 把建議卡改成兩段式後檢索不再擋在生成前面，短逾時只剩下「100% 逾時、
 * 建議卡永遠拿不到引用」這一個後果。**MUST NOT 為建議卡另立第二個數字**——
 * `test/contract-guards.test.ts` 以原始碼掃描守著它不復活（連別名也不行：
 * 別名日後照樣會被改成別的數字，那正是 002 走過一次的路）。
 */
