 ...
 *    **結案中＝一個不能按的狀態徽記**），畫布如此。「離開對話」與接手的兩種模式選項
 *    留在展開態 —— 那些是有後果、需要讀完輔助說明才按的動作，不該塞進一條 38px 的窄列。
 *
 * ⚠️⚠️ **結案中 MUST NOT 再顯示「結案」鍵**（2026-09-08 手動驗收回報）。
 *      此前這裡只有「未接手／已接手」兩種分支，於是進入結案流程之後把資訊列收起來，
 *      畫面又出現一顆「結案」—— 而展開態明明是「取消結案 ＋ 結案中…」。
 *      客服會以為剛才那次沒按到而再按一次，**而再按一次不會報錯**
 *      （對已在結案中的對話，那是一次無效操作），
 *      所以這個矛盾只會停在畫面上，不會在任何地方留下痕跡。
 *
 * ⚠️ 結案中放的是**徽記不是按鈕**（畫布逐字：`--navy-soft` 底、`--navy-soft-bd` 框、
 *    radius 20px、旋轉 `loader-2`）。做成 disabled 按鈕也擋得住點擊，但那仍然長得像
 *    「這裡本來可以按」；徽記從形狀上就說明它是狀態。
 *    ⚠️ **「取消結案」不放進來** —— 畫布把它留在展開態，**2026-09-08 使用者裁示維持如此**
 *    （提出過「收合態是否也該能取消」，結論是不放）。要取消就按右邊的展開鈕，
 *    那顆在任何寬度下都不會消失（見檔尾 container query 的說明）。
 *    ⚠️ 要重啟這個討論，**重啟條件是先解決「多一顆按鈕要從哪裡讓位」** ——
 *    這一列只有 38px 高，而它的寬度不是自己決定的（中欄被左右兩欄夾著）。
 *    不要只加按鈕而不動讓位清單：那會退回 2026-09-01 修掉的溢出到右欄的 bug。
 *
...
defineProps<{
  ...
  joined: boolean
  /** 這個對話是否正在結案（specs/006）—— 由頁面的單一判定來源傳入，不在這裡自己推 */
  closing: boolean
  busy: boolean
}>()
...
<template>
  ...
  >
    ...
    <!-- ⚠️ 結案中：不能按的狀態徽記，不是（disabled 的）按鈕 —— 理由見檔頭 -->
    <span
      v-else-if="closing"
      class="flex shrink-0 items-center gap-1.5 rounded-[20px] border px-[9px] py-[2px] text-[0.8438rem]"
      :style="{
    ...
    >
      <UIcon name="i-lucide-loader-2" class="size-[11px] shrink-0 animate-spin" />
