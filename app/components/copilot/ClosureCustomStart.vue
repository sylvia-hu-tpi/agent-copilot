 ...
 *
 * ⛔ **彈窗裡不顯示「約 N 則」預估**：它需要額外一次訊息掃描，而套用之後真正的則數
 *    會由 `draft.period.messageCount` 呈現。與其顯示一個可能與最終結果不同的估算，
 *    不如不顯示。
 *    ✅ 2026-09-08 版畫布已採納：元件裡的 `customEst` 與 2b「B1 從未結案過」說明卡的
 *    相同描述都已移除，彈窗現在是「月曆 → 時分 → 可選範圍 → 取消／以此起算並重新產生」，
 *    與這裡逐字一致（舊 `DESIGN_FEEDBACK.md` D-4 已結清）。
 *
