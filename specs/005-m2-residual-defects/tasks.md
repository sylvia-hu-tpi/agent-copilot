...
- [x] T050 [US4] 依 FR-019 的判準做決定並寫進 `docs/ARCHITECTURE.md`（證據寫入 §8.2b「並行度掃描跑完了」與 §18 M2 情緒那條；**2026-09-03 使用者裁決：維持 `SENTIMENT_CONCURRENCY = 3`、情緒 15 秒門檻不改判**）。
...
- [x] T058 依 [quickstart.md](./quickstart.md) 逐項執行手動驗收（**2026-09-03 全數通過**）：US1 真實雙分頁（關一個、另一個 4 秒內收到）、送出不閃重插、LEAVE 對所有分頁生效、背景分頁節流 10 分鐘後心跳 upsert（**已訂正為「切回等一拍心跳→關掉前景分頁→送訊息」的可鑑別版本**）。⚠️ 原列的「斷網而非關分頁」存活兜底**已從手動清單移除**——該情境在設計上驗不到（要斷網的正是觀察者自己、整條路徑零 log、無 debug 端點），改由 `test/connection-counting.test.ts`（28 tests）涵蓋，理由寫在 quickstart
...
**下次續跑**：
1. ✅ **T058 手動驗收已於 2026-09-03 全數通過**（真實瀏覽器、stable 環境）——**本規格已無剩餘任務**。
   ⚠️ 原本的第四項「斷網後觀察存活兜底」**已從手動清單移除**，改由
   `npx vitest run test/connection-counting.test.ts`（28 tests）涵蓋 —— 該情境在設計上驗不到：
   要斷網的正是觀察者自己，且整條路徑零 log、無 debug 端點。詳見 quickstart 該節。
2. ✅ **已裁決（2026-09-03，使用者）**：並行度維持 `SENTIMENT_CONCURRENCY = 3`（採納 FR-019 判準
   得出的建議）。證據見 `docs/ARCHITECTURE.md` §8.2b，程式碼與門檻皆未動。
3. ✅ **已裁決（2026-09-03，使用者）**：§18 M2 的情緒 15 秒門檻**不改判**，維持「未通過」的記載 ——
   本次檔位 3 量到 41/45 ＝ 91%（通過），但 2026-09-02 的同口徑量測是 78%，
...
## 2026-09-03 收尾狀態（給下一次續跑的人）

**65/65 完成、全套驗證綠**（typecheck；43 檔 **555 測試**——含新增的探測閂回歸測試；build；`smoke:flow`；
`smoke:realtime` 含新加的「同一客服關掉一個分頁、另一個 4 秒內仍收到新訊息」真實情境，實測 2.7 秒）。

**T058 手動驗收已全數通過**（2026-09-03，真實瀏覽器、stable 環境，步驟在 `quickstart.md` US1 段）：
✅ ① 同帳號兩分頁進同一段對話、關一個、另一個 4 秒內看到新客戶訊息 —— **2026-09-03 通過**；
✅ ② 留著的分頁送訊息不閃重插 —— **通過**；
✅ ③ 任一分頁按「離開對話」→ 所有分頁面板一起消失 —— **通過**；
🚫 ④ 斷網後的存活兜底 —— **已從手動清單移除**，改由 `test/connection-counting.test.ts`（28 tests，
   2026-09-03 全綠）涵蓋。理由見 `quickstart.md`：要斷網的正是觀察者自己，且整條路徑零 log、
   無 debug 端點，原句「第一層輪詢停止」也與實作不符（是空轉，不是停止）；
✅ ⑤ 背景分頁節流後心跳 upsert 重建登記 —— **通過**（用下述訂正後的可鑑別版本跑的）。
   ⚠️ **原寫法（切背景 10 分鐘→切回→確認仍收得到訊息）會假通過**：背景分頁的 SSE 不被節流
   （EventSource 是 server push，瀏覽器只節流計時器），watchers 還在、fan-out 照送；
   只要前景分頁的登記還在，輪詢就取得到 token，**心跳寫成 no-op 也會過**。
   訂正為：切回後等一拍心跳（20～30 秒）→ **關掉前景分頁** → 送客戶訊息，
   讓受測分頁成為該組織僅剩的登記，upsert 才是唯一能決定成敗的變數。詳見 quickstart。

⚠️ **⑤ 前兩次的結果作廢（不是失敗），第三次才有效** —— 都栽在 dev server 被重啟：
- 第一次：10:30 在使用者等待的 10 分鐘當中編輯了 `server/services/copilot-analysis.ts`（只加註解），
  觸發 Nitro 熱重啟 → `MemoryStateStore` 的 process-local session 全部清空 →
  `/api/auth/me` 回 401 → `stream.ts` 的 `probeSession()` 把**兩個分頁**一起導去 `/login`。
- 第二次：10:54 **另一個 session** 跑了會重建 `.nuxt/` 的指令（`nuxt prepare`／`typecheck`／`npm test`
  都會），同樣完整重啟。這次症狀不同：兩個分頁**卡在「連線中斷，重新連線中…」而不跳登入頁**，
  成因是下方「探測閂」那個缺陷。
- 第三次：由使用者自己在終端機起 `npm run dev`（看得到 `Nitro server built` 即時判斷是否被重啟），
  並先與另一個 session 協調不跑驗證指令 → 順利跑完。

📌 **三種失敗長相的判斷指紋**（前兩種都不是 ⑤ 的結果）：
| 看到的 | 意義 |
|---|---|
| **只有背景過的那個分頁**收不到訊息、另一個正常、且連線是綠的 | ✅ 真正的 ⑤ 失敗（心跳 upsert 壞了） |
| 兩個分頁**一起跳回登入頁** | dev server 重啟、session 被清 → 作廢 |
| 兩個分頁**一起卡在「重新連線中…」** | dev server 重啟 ＋ 探測閂缺陷 → 作廢；F5 可脫困。⚠️ **探測閂已於同日修掉**（見下方），修後這一格的症狀會變成「延遲數十秒後導去登入頁」，不再永久卡住 |

📌 **手動驗收進行中 MUST NOT 編輯 `server/**`／`shared/**`／`nuxt.config.ts`**，
且 MUST 先與其他 session 協調**不要跑 `npm test`／`typecheck`／`build`**（都會重建 `.nuxt/`）。
⚠️ 完全免疫的做法是改跑 `npm run build && npm run preview`（`.output/server/index.mjs` 不監控檔案）。

**兩件裁決已於 2026-09-03 拍板（證據都在 `docs/ARCHITECTURE.md` §8.2b／§18 M2，程式碼與門檻都沒動）**：
- ✅ 並行度：3／4／5 掃描（每檔位 n=45）顯示 4 與 5 連總時間都沒改善且失敗率上升 → **維持 3**。
- ✅ 情緒 15 秒 p90：本次檔位 3 量到 41/45＝91%（9/02 同口徑 78%）→ **不改判**，要翻案需另一個獨立時段的 n=45。

**🆕 ✅ T058 過程中發現並**順帶修掉**的缺陷（2026-09-03）——
「探測閂」讓分頁永久卡在『重新連線中…』**：

**症狀**：`app/stores/stream.ts` 的 `onerror` 在累積 2 次失敗時探測一次 session，但
**`sessionProbed = true` 是在發動探測「之前」就設的**，而它只有「連線成功」或
「明確 `disconnect()`」會重置。於是斷線當下若 server 正在重啟，探測拿到的是
**網路錯誤而非 401** → `if (code !== 401) return` 直接返回 → 閂已鎖死。
等 server 起來、`/api/stream` 因 session 已清而回 401 時，`onerror` 雖然繼續累加失敗，
**卻永遠不會再探測第二次** → 分頁無限重試（退避上限 30 秒）、**永遠不會導去登入頁**，
只能手動重新整理才脫困。T058 第二次嘗試時實際踩到。

**為何順手修**：與 005 US1 同屬連線層，且是同一族的靜默失效；005 本來就有
「順帶修掉 `runBlockDeduped()` rerun 重跑第一次閉包」的先例。

**修法**：一次性旗標 → **會重新武裝的門檻** `nextProbeAtFailures`
（新常數 `SESSION_PROBE_RETRY_EVERY_FAILURES = 3`）。探測沒有定論（非 401）或確認 200 時，
都把門檻往後推 3 次失敗而不是從此不再問；另加 `probeInFlight` 避免疊送。
⚠️ **不能直接把閂拿掉** ——「一輪斷線只探測一次，不要每次重試都打一發 `/api/auth/me`」
是刻意約束，`test/nuxt/stream-store.test.ts` 已有斷言守著。退避上限 30 秒，
因此穩定狀態下約每 90 秒才探測一次。

**回歸測試**：`test/nuxt/stream-store.test.ts` 新增
「探測沒有定論後 MUST 重新武裝」，與既有的「探測失敗但不是 401 時不得登出」
構成**夾擊條件**（只有後者時，「問一次就永遠不再問」的實作也會通過）。
✅ **已驗證這條測試會紅**：暫時把該分支改回 `Number.POSITIVE_INFINITY` 重跑，
13 條中**只有新測試變紅**，其餘 12 條全綠。

**已知但不在 005 範圍、可另開工項**：杜撰成因是模型抄文件內文的正式 SOP 編號（我方交的是
`knowledge-fallback-<hex>` 代用碼）→ 真正的槓桿是讓檢索結果帶真實識別碼（`IMBRACE_QUESTIONS` 0-3g ②）；
`withRetry()` 逾時不取消已送出的呼叫，實際並發＝設定值＋放棄的呼叫；以及本區塊上方 code review
列出的設計層項目。**兩個裁決與 T058 手動驗收皆已完成，`m2-done` tag 可押。**
