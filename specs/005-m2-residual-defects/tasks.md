...
**64/65 完成、全部 commit、全套驗證綠**（typecheck；43 檔 554 測試；build；`smoke:flow`；
`smoke:realtime` 含新加的「同一客服關掉一個分頁、另一個 4 秒內仍收到新訊息」真實情境，實測 2.7 秒）。

**只剩 T058 手動驗收**（真實瀏覽器、stable 環境，步驟在 `quickstart.md` US1 段）：
① 同帳號兩分頁進同一段對話、關一個、另一個 4 秒內看到新客戶訊息；② 留著的分頁送訊息不閃重插；
③ 任一分頁按「離開對話」→ 所有分頁面板一起消失；④ 斷網（不是關分頁）45 秒後登記被回收；
⑤ 背景分頁放置 10 分鐘以上再切回仍收得到訊息（心跳 upsert）。

**兩件待你裁決（證據都在 `docs/ARCHITECTURE.md` §8.2b／§18 M2，程式碼與門檻都沒動）**：
- 並行度：3／4／5 掃描（每檔位 n=45）顯示 4 與 5 連總時間都沒改善且失敗率上升 → 建議維持 3。
- 情緒 15 秒 p90：本次檔位 3 量到 41/45＝91%（9/02 同口徑 78%），刻意不改判，要翻案需另一個獨立時段的 n=45。
...
列出的設計層項目。`m2-done` tag 待 T058 與兩個裁決之後再押。
