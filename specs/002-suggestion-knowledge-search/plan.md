...
**Performance Goals**: JOIN 後 90% 情況 3 秒內建議卡區塊出現並標示產生中，10 秒內完整呈現
（SC-001，與 001 的摘要/情緒門檻對齊，因為冷啟動是同一批 `Promise.all`）；知識庫快查 90% 情況
...
⚠️ **2026-08-27 依實測修訂**：原本只有單一的 8 秒（推導自 SC-002 的 10 秒門檻，留 2 秒給 BFF 往返
與渲染）。實測九次取樣最快 13.0 秒、最慢 24.9 秒，8 秒在生產路徑上 100% 逾時，SC-002 的門檻已改寫
為 25 秒。兩條路徑之所以**必須**用不同數字：快查是客服主動發起、畫面有骨架的同步查詢，等 20 秒
可接受；建議卡走「先檢索再生成」的串行流程且受 SC-001 的 10 秒約束，若也用 30 秒會讓建議卡遲到
30 秒。取樣數據與完整理由見 `server/services/knowledge/agent-knowledge-provider.ts` 的常數註解；新增背景並行上限
