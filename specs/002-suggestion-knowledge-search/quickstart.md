...
- 知識庫檢索逾時上限為 30 秒（`KNOWLEDGE_SEARCH_TIMEOUT_MS`，2026-08-27 依實測由 8 秒調整）：
  ...
  但語意與處置完全不同。
- **快查會等 9～23 秒**（2026-08-29 兩組實測：固定查詢 n=12 中位 11.9／p90 16.9／最慢 20.1 秒；真實對話查詢 n=10 中位 18.4／最慢 22.9 秒，另有 1 次逾 30 秒上限而降級為 degraded。原記 13～25 秒、9～20 秒皆已訂正。非缺陷）：iMBrace 的知識庫檢索延遲就是這個量級，等待期間
  顯示骨架。驗收時請以「有沒有骨架、最後有沒有結果」為準，不要因為慢就判定失敗。
- ~~**建議卡目前拿不到知識庫引用，一律標示「未引用知識庫」**~~ —— **已由 004 解決（2026-08-29）**。
  當時的成因是建議卡另立了 8 秒的檢索短逾時（為保護 SC-001 當時的 10 秒門檻），而實測檢索最快
  9.4 秒、0/12 落在 8 秒內，那個上限的實際效果就是「永遠拿不到引用」。
  `specs/004-progressive-citations` 把前景建議卡改為**兩段式**：第一段不等檢索先出卡並標示
  「**尚未**引用知識庫」，檢索有命中時第二段以命中結果重新生成整批並自動換上（標示轉為有來源）。
  短逾時常數已刪除，建議卡與快查共用 `KNOWLEDGE_SEARCH_TIMEOUT_MS`（30 秒）。
  ⚠️ 驗收時請改依 004 的序列判讀：`pending`（尚未引用・檢索中）→ `cited`／`none`，
  完整規則見 `specs/004-progressive-citations/contracts/progressive-suggestion-events.md`。
  「**未**引用知識庫」自此只代表最終確定沒有依據，不再是「查不到就是這樣」的常態。
