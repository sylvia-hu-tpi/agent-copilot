...
- [x] T052 依 quickstart.md 完成人工驗收：US1-A 靜置 10 分鐘統計、US2-B 面板消失逐項清單、US2-C 兩瀏覽器（含用 EventStream 確認未 JOIN 端真的收不到，**含連線當下的快照**）、US2-E 收合不等於離開（含切換對話與重新整理後偏好仍在，FR-017a／FR-017b）、SC-007 找一位未參與本規格的同事讀文案。⚠️ 另加一項：**同一位客服開兩個分頁、在其中一個按下離開**，另一個分頁的面板 MUST 跟著消失（T032a）
  ...
  - [x] **US2-C**（2026-09-02 通過，兩位**不同**的客服帳號、兩個隔離的瀏覽器 session）：A JOIN、B 全程不 JOIN —— A 面板照常更新；B 整欄面板不呈現、中欄延伸至右緣；B 的 EventStream 自**連線建立起**全程零則 `summary.updated`／`sentiment.updated`／`suggestion.updated`（含 attach 當下的快照）；`messages.appended`／`presence.updated`／`control.updated`／`stream.heartbeat` 照常。⚠️ **重跑時 MUST NOT 以同一帳號兩分頁替代**：那是 T032a 的情境，驗的是不同的東西，兩者不可互換
  - [x] **SC-007**：✅ **已於 2026-09-07 由 `specs/006-closure-handoff-summary` SC-005／T052 重新驗證通過（3 位未參與者、3/3）** —— 下方是 2026-08-28 當時的結案理由與「要重新提出」的但書，**該但書已執行完畢**，保留為歷史紀錄。⚠️ **依使用者決定結案（2026-08-28），非依原判準通過** —— 原判準要求「一位**未參與本規格**的同事」讀畫面後能自行說出「一個留紀錄、一個不留」，**該外部讀者測試從未執行**。結案理由：這行文案是階段性的（`exitHint` 描述的是 M3 才會實作的結案流程，見下方附錄），且 M2 全部完成後 PM／SA 會針對細部文案與功能提出微調方案，該關卡涵蓋範圍更廣。⚠️ **後續若 PM／SA 的審查未涵蓋這兩個出口的可讀性，這一項要重新提出** —— 它是憲法 8.1「差別 MUST 由文案讀得出來」在本規格唯一的驗收點
