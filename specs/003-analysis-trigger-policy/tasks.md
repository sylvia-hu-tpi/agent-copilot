...
- [ ] T004 新增 `test/contract-guards.test.ts`：斷言 `shared/` 底下不存在 `failedBatches` 字串（契約 1.1 的可執行版本，取代人工 grep）。⚠️ **MUST NOT** 併進 `test/component-names.test.ts` —— 那支測試在找不到 `.nuxt/components.d.ts` 時會整支 skip（乾淨 checkout 上是正常情況），本守衛會跟著被靜默跳過，那正是它要防的失效形態
...
- [ ] T009 在 `server/utils/stream-control.ts` 的 `createWatchRegistry()` **回傳物件上新增 `isJoined(conversationId): boolean` 方法**（讀 `WatchRegistration.joined`，比照既有的 `has()`），供 T028 的推播過濾使用 —— 明確標註這是不變式 C 的唯一資料來源。⚠️ **MUST NOT** 寫成模組層 `export function`：`watched` 是 closure、每條連線一份，模組層函式只能改用全域 Map，那會讓所有連線共用一份 JOIN 狀態（動工前必讀 #2）
- [ ] T010 [P] 在 `server/services/copilot-analysis.ts` 新增失敗批次記憶的三個純存取函式：`readFailedBatch(state, block)`、`markFailedBatch(state, block, lastMessageId)`、`clearFailedBatch(state, block)`，並匯出供測試直接引用（data-model.md §1）
...
- [ ] T013 [P] [US1] 新增 `test/analysis-failure-memory.test.ts`：同一 `(區塊, 最後一則訊息 id)` 失敗後再次觸發 → 不執行；`retryBlock()` 後 → 執行；`runColdStart()` 後 → 執行；成功後記憶被清除；**`beginAnalyzing()` MUST NOT 清除記憶**（data-model.md §1 的反例）；**FR-011**：記憶隨 `CopilotAnalysisState` 一起消失（狀態不存在時讀取回 `null`，不另有保存期限）
- [ ] T014 [P] [US1] 在 `test/copilot-analysis.test.ts` 補上 FR-009 併發去重：同區塊連續觸發三次 → 只執行兩次（當次 + 一次合併的 rerun）；**rerun 那次仍會過失敗批次記憶檢查**（data-model.md §3 第 3 點）
- [ ] T015 [US1] 新增 `test/analysis-trigger-integration.test.ts`（對假 gateway，沿用 `test/mock-gateway.ts`）：AI 端點恆回 500 → JOIN → 模擬 30 次心跳（不使用真實計時）→ 斷言分析嘗試總數 ≤ 1 輪（SC-001 的自動化版本）
...
- [ ] T017 [US1] 在 `server/services/copilot-analysis.ts` 的 `finishBlockError()` 新增第四個參數 `batchLastMessageId: string | null`，失敗時寫入失敗批次記憶；為 `null` 時不寫入（沒有可判定的批次，寧可下次再試也不要用假的鍵擋住未來的分析，data-model.md §1）
- [ ] T018 [US1] 在 `server/services/copilot-analysis.ts` 的三個分析入口（`analyzeSummary` / `analyzeSentimentBatch` / `analyzeSuggestions`）算出該批的 `lastMessageId` 並向下傳給 `finishBlockError()`；同時在 `beginAnalyzing()` **之前**加入失敗批次記憶檢查，命中則直接 return（FR-005、FR-006）
- [ ] T019 [US1] 在 `server/services/copilot-analysis.ts` 用 T011 的 `runBlockDeduped()` 包住三個分析入口，實作 FR-009 的合併與 rerun；rerun 前 MUST 重新讀一次失敗批次記憶
- [ ] T020 [US1] 在 `server/services/copilot-analysis.ts` 的 `retryBlock()` 開頭清除該區塊的失敗批次記憶（FR-008），在 `runColdStart()` 開頭清除全部三個區塊的失敗批次記憶（FR-015）
- [ ] T021 [US1] 以 `grep -rn "setTimeout\|setInterval" server/services/copilot-analysis.ts` 與 `grep -rn "自動重試\|秒後重試" app/ i18n/ server/` 確認**沒有**任何自動退避重試的計時器或「X 秒後自動重試」文案（FR-010）；若有殘留一併移除。⚠️ 這是「確認型」任務，**MUST 把實際執行的指令與輸出貼進該 phase 的 commit 說明**，否則沒有任何痕跡能證明它跑過
...
- [ ] T024 [P] [US2] 新增 `test/copilot-panel-collapse.test.ts`：可見性判定 = `viewerJoined`（**MUST NOT** 由 Block 是否 `empty` 推出）；收合偏好以 `ac.copilotCollapsed.${conversationId}` 為鍵、per 對話各自記；未存過時預設展開（FR-016、FR-017a）；**FR-017b**：切換收合狀態 MUST NOT 送出任何請求、MUST NOT 改動 `joined`，也 MUST NOT 影響分析排程 —— 收合是純視覺狀態（US2 AC#6）
...
- [ ] T026 [US2] 在 `server/services/copilot-analysis.ts` 匯出 `cancelPendingAnalysis(conversationId)`（`clearTimeout` 並刪除 `debounceTimers` 條目）（FR-013 的清理層，data-model.md §4）。**本任務只做匯出，呼叫點在 T027a**
- [ ] T027 [US2] 在 `server/api/stream.get.ts` 讓 `attach()` 內的**兩條**補跑路徑都受 `isJoined()` 門檻約束：① `sendAnalysisSnapshotAndResume()` 裡的 `runIncremental`；② 與它並列呼叫的 `catchUpSummaryIfStale()`（前景切回補跑摘要，FR-004 已補上「限已 JOIN」限定語）。⚠️ **快照本身對已 JOIN 的連線照送**（001 FR-010 不得退步），但**對未 JOIN 的連線整段跳過**——快照走的是 `send()` 不經 `forward()`，T028 的過濾管不到它（FR-003、FR-016a、契約不變式 C）
- [ ] T027a [US2] 在 `server/api/stream.get.ts` 的 `attach()` 內、`watchConversation()` 之後檢查 `runtime.messageSource.isJoined(conversationId)`；為 `false` 即呼叫 `cancelPendingAnalysis(conversationId)`（FR-013 的呼叫點）。⚠️ 選這一層的理由：客服按下離開時前端會**立刻**送一次 `beat('viewing')`（`useConversationView.leave()`），因此 `watch` 會在一個往返內帶著 `joined: false` 抵達，SC-002 的 5 秒門檻有餘裕；而 `leave.post.ts` 維持「一行未動」（plan.md 憲法七的檢核前提），`server/sources/` 也不必反向相依 `server/services/`
- [ ] T028 [US2] 在 `server/api/stream.get.ts` 的 `forward()` 加入分析事件過濾：以 T009 的 `watchers.isJoined(conversationId)` 判斷，未 JOIN 時丟棄三個分析事件、其餘一律照送（FR-016a）。⚠️ 過濾 `stream.heartbeat` 會直接斷線 —— 清單以契約不變式 C 的表格為準
...
- [ ] T034 [US2] 在 `test/copilot-panel-collapse.test.ts` 補上憲法 8.4 的斷言：LEAVE → 面板消失 → Composer 草稿仍在。若面板隱藏連帶重建了頁面元件而清掉草稿，改為只卸載面板子樹。⚠️ MUST 是自動化斷言而非人工目視 —— 草稿遺失只在特定卸載路徑下發生，人工驗一次不代表下次改版還成立
...
- [ ] T036 [US3] 確認 T018 的失敗批次記憶檢查是以「該批**最後一則**訊息 id」比對而非對話層級或時間窗 —— 這是自癒的支點，寫成其他鍵會讓 US3 完全失效（data-model.md §1）。⚠️ 產出物：T035 的斷言即為證據；若 T035 已涵蓋則在此打勾並註明，不另做人工檢查
...
- [ ] T038 [P] [US4] 新增 `test/copilot-retry-all.test.ts`：只對 `status === 'error'` 的區塊發出請求；無任何 error 時按鈕為不可按；已成功的區塊 MUST NOT 被重跑（FR-018）。⚠️ **MUST NOT** 塞進 `test/copilot-panel-collapse.test.ts` —— 檔名與內容不符，日後找不到
...
- [ ] T040 [US4] 在 `app/components/copilot/PanelHeader.vue` 接上「全部重試」按鈕：`disabled` 條件為「三個區塊都不是 `error`」，須可鍵盤操作（憲法 8.2）；不可按狀態 MUST 同時以 `disabled`／`aria-disabled` 表達，MUST NOT 只靠降低對比度（憲法 8.1）。**刻意不做樂觀 disable** —— 往返期間按鈕仍可按是預期行為，重複按由 FR-009 吸收（FR-019、research.md 決策 7）
- [ ] T041 [US4] 在 `test/copilot-retry-all.test.ts` 補一條斷言：各區塊自身的重試按鈕**未被加上任何互鎖邏輯**（FR-019）—— 按下「全部重試」後，區塊自己的 `disabled` 條件仍只看 `status`，不看「全部重試是否在進行中」。既有的「僅 error 可按」規則已足夠，區塊轉 `analyzing` 會讓按鈕自然失效
...
- [ ] T049 執行 `CLAUDE.md` 要求的舊說法掃描：`grep -rn "保留但凍結\|凍結標示" docs/ specs/`、`grep -rn "每 20 秒\|無限重試\|3,780" docs/`、`grep -rn "自動重試" docs/ARCHITECTURE.md`、`grep -rn "380" docs/DESIGN_TOKENS.md`（T031 已把預設寬度改為 420）。⚠️ **這一步是本專案最常犯、代價最高的錯誤，掃到第三輪為止**。⚠️ spec.md 的 Clarifications 刻意保留了被推翻的「保留但凍結」原文（含刪除線與推翻註記）—— 那是**故意留下的**，掃到它時 MUST NOT 清掉
- [ ] T050 [P] 確認 `docs/IMBRACE_QUESTIONS.md` G-2（rate limit 書面規格）**不撤回** —— 本規格大幅降低呼叫量使該題急迫性下降，但問題本身仍未獲答覆
- [ ] T051 [P] 核對 `docs/DESIGN_TOKENS.md` 是否需為面板收合態與兩個離開出口新增 token；**並 MUST 更新 §7.1 末段的「⚠️ 實作端仍是 380」** —— T031 已把 `copilotWidth` 預設值改為 420，該行若不改就成為新的過期敘述（`CLAUDE.md` 第一級警告的形態）
