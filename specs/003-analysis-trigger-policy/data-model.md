...
export interface FailedBatch {
  ...
  count: number
  /**
   * 手動重試（FR-008）或冷啟動（FR-015）已把這一批**放行**：門檻不再擋它，但 `count` 保留。
   * ⚠️ 見下方「為何是放行而不是刪除」—— 這一欄是實作階段（2026-08-28）修正的設計。
   */
  released?: boolean
}
...
| 動作 | 時機 | 函式 | FR |
|---|---|---|---|
| **讀**（決定要不要跑） | 各區塊分析**進入點**，在 `beginAnalyzing()` 之前 | `isBatchAlreadyFailed()` | FR-006 |
| **寫**（記錄失敗） | `finishBlockError()`，需由呼叫端傳入該批的 `lastMessageId` | `markFailedBatch()` | FR-005 |
| **放行**（手動重試） | `retryBlock()` | `releaseFailedBatch()` | FR-008 |
| **放行**（冷啟動／重新 JOIN） | `runColdStart()` | `releaseFailedBatch()` | FR-015 |
| **清**（分析成功） | 各成功寫入路徑 | `clearFailedBatch()` | 隱含 |

⚠️ **只有「分析成功」是真的刪除整筆**，另外兩處是把 `released` 設為 `true`。
門檻的判定式因此是「同一批 **且** 尚未放行」（`isBatchAlreadyFailed()`）；
而 `markFailedBatch()` 在重新記錄失敗時會**一併清掉 `released`** —— 放行只有一次機會，
放行後那次仍然失敗就重新擋住，否則同一批會被無限重試，FR-006 等於失效。

### 為何是放行而不是刪除

> ⚠️ **本節是 2026-08-28 implement 階段的設計修正。** 本文件原先在上表寫的是「清（手動重試）」，
> 與 `count` 的定義「手動重試也失敗時遞增」**互相矛盾** —— 直接刪掉整筆的話，`count` 每次都從 1
> 重新起算，而 `count` 唯一能超過 1 的路徑正是「手動重試也失敗」，該欄位會變成恆為 1 的死欄位。
> 放行旗標讓兩個要求同時成立，代價只是一個布林。

⚠️ **`released` MUST 是狀態，MUST NOT 做成呼叫端的參數。**
分析入口有同區塊併發去重（§3 的 `runBlockDeduped()`），手動重試很可能被**合併**進一次
已在進行中的分析 —— 屆時真正執行的是**先前那個**閉包。若把「這次略過門檻」寫成參數傳進去，
它會在合併路徑上遺失，症狀是「剛好有分析在跑的時候按重試，按了沒反應」：
按鈕有回應、請求 200、但什麼都沒發生，且不會報錯。

⚠️ **`beginAnalyzing()` 既不是清除點、也不是放行點**。直覺上會想寫在那裡（「開始分析就清掉」），但它是每次分析的
