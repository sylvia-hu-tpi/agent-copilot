...
**唯一的伺服器端面板變更**是 FR-016a：`forward()` 對未 JOIN 的連線過濾掉三個分析事件
（其餘事件一律照送，中欄不受影響），判斷資料重用決策 1 已經要新增的 `WatchRegistration.joined`。
...
「Session 2026-08-28 補充」與 tasks.md 附錄。

⚠️ **本規格的釐清過程發現一項待修憲事項**：憲法 5.3 的「以 `conversation_id` 為唯一鍵覆蓋」
與「同一對話可被不同人在不同時間結案多次」牴觸，需改為 uuid 主鍵 ＋ 以草稿 id 為冪等鍵。
**MAJOR 變更，建議獨立進行、不併入本規格**（003 完全不寫入 Data Board；憲法 B.4 建議
MAJOR 變更在里程碑交界進行，M2→M3 交界正合適）。詳見 spec.md「待修憲事項」與 tasks.md T053。
...
`failedBatches`；⚠️ 刻意不放進三個 Block —— SSE 事件送的正是整個 Block，放進去會讓失敗記錄
...
- 單元：心跳去重（相同 `{priority, joined}` 不 attach／任一改變則 attach）、失敗批次記憶的存取與三個
  清除點、同區塊併發去重與 rerun、`isJoined()` 聚合、`forward()` 的分析事件過濾、收合偏好的讀寫
...
（5 個 server 檔、3 個 app 檔）但位置關鍵——`watch()` 與 `runIncremental()` 是整條 Copilot 管線的
...
| **八、介面與無障礙** | 面板可見性／收合按鈕／「全部重試」／兩個離開出口 | ✅ 通過。8.1：面板的存在與否本身不是「狀態以顏色表達」的情形；「離開對話」與「結案」的差別 MUST 以文案而非僅靠視覺層級表達（SC-007）；8.2：收合按鈕與「全部重試」須可鍵盤操作，比照既有各區塊重試鈕；8.4：**面板隱藏 MUST NOT 影響 Composer 草稿**——草稿在中欄，與面板無關，但需在測試中明確覆蓋（LEAVE→面板消失→草稿仍在）；8.5：全部新文案入 i18n |
...
├── api/
│   ├── stream.get.ts              # attach() / sendAnalysisSnapshotAndResume()：補跑改受 isJoined() 門檻
│   │                              # ⚠️ forward()：對未 JOIN 的連線過濾三個分析事件（FR-016a）
│   └── conversations/[id]/
│       └── copilot/retry.post.ts  # 不改形狀；經 retryBlock() 連帶清除失敗記憶（FR-008）
├── services/
...
唯一的新檔案是一個 Vue 元件與三份測試——這反映本規格的性質：改動集中在既有模組的判斷式，
而非新增模組。`shared/types/` 刻意**不動**（失敗記憶是 server-only，見 Technical Context 的 Storage）。
