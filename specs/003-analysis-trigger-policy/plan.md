...
既有的延遲門檻（001 的重連快照 2 秒、001 SC-001 的 3 秒骨架、smoke 的 4 秒）**MUST 不退步**（SC-005）。
⚠️ 原文寫的是「002 SC-001 的 3 秒／10 秒」，兩處都不準：3 秒骨架出自 **001** SC-001，
而 002 SC-001（建議卡）的門檻早於 2026-08-29 由 10 秒改為 **20 秒**、001 SC-005 也於
2026-09-01 拆成「摘要 10 秒／情緒 15 秒」。**本規格的回歸判準是「不退步」，不是那些數字本身**
—— 數字的正典在各自的 spec 與 `docs/ARCHITECTURE.md` §18，此處不再複述，以免又過期一次。
