...
| 形態 | server 行為 | HTTP | 前端 |
|---|---|---|---|
| 逾時（30 秒，R3.12） | 不重試（客服自己決定要不要重按） | 504 | 回 `ready` ＋失敗提示，草稿在、取消鍵恢復可用 |
| 平台 4xx | 原樣轉為失敗，訊息去識別化 | 502 | 同上 |
| 平台 5xx | 同上 | 502 | 同上 |
| 200 但回查不存在 | **當作失敗**（R3.5） | 502 | 同上 |

⚠️ 四種在前端 MUST 是**同一條路徑**（回 `ready` ＋掛錯誤）。
分開處理只會讓其中一條被漏掉，而漏掉的那條會顯示成功。
