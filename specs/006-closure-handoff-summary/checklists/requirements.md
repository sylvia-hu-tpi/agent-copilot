...
- **FR-021a～FR-021g 只定行為與資訊，不定版面** —— 這是刻意不在規格裡替設計做決定。
  涵蓋區間選擇器原本在畫布上不存在，比照交接摘要的同一標準先進 Design 畫布；
  畫布已於 2026-09-03 補上（artboard 2b），版面一律以畫布為準。
...
2. ✅ **涵蓋區間選擇器的設計稿** —— **已於 2026-09-03 進畫布**，新增 artboard **2b**
   （結案摘要區塊的狀態矩陣）。逐字規格已擷取至 `docs/DESIGN_TOKENS.md` §7.5。
   核對結果：畫布與 FR-021 系列逐條對得上，且有兩處比規格更嚴謹 ——
   0 則候選直接不可選（非只是預設跳過）、則數 > 150 轉 `--warn` 色。
