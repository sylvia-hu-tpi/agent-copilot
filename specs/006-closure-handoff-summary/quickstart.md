...
⚠️ **狀態（2026-09-04）：未執行、由使用者放行**，見 §6.4。本節保留的是「補做時怎麼做」——
...
| **SC-005** 3 位未參與者說得出兩個出口的差別 | ⏸ **未執行，使用者放行**（2026-09-04） | 3 人訪談未進行，改由使用者後續與 PM 確認；`tasks.md` T052 已勾選＝本規格不再追，**不等於通過**。見 6.4。⚠️ 這是 003 SC-007 的重跑，MUST NOT 因為行為已補上、或因為 006 已結案就視為通過 |
...
### 6.4 人工驗收（T046 ✅ 已執行｜T052 ⏸ 放行未執行｜T053、T054 ✅ 2026-09-07 第二輪重跑通過）

> ⚠️ **T046 已於 2026-09-04 執行完畢（使用者知情後），紀錄見 6.4.1。**
> 其餘三項**無法自動化**：它們驗的是「人讀不讀得懂」「一條不存在的路徑真的不存在」
> 與「真實瀏覽器 ＋ 真實 AI 的整段流程」。
>
> ⚠️ **T052 於 2026-09-04 由使用者決定放行**（`tasks.md` 已勾選、改與 PM 確認）。
> 勾選關閉的是**任務**，不是 **SC-005** —— 3 人訪談未進行，這條驗收條件至今沒有證據。
> 對外回報 006 完成度時 MUST 一併說出這點。

| 任務 | 內容 | 為什麼不能自動化 |
|---|---|---|
| **T052**（SC-005）⏸ **未執行，已放行** | **2026-09-04 使用者決定放行並勾選 T052**，3 人訪談未進行，改由使用者後續與 PM 確認。素材已備妥可直接沿用（截圖裁切到只剩兩顆按鈕 ＋ 那行文案；問句見 §2 SC-005），要補做時不必重新準備。原內容：找 **3 位未參與本專案**的人，只給看兩顆按鈕與 `conversation.exitHint`，**在按下之前**請他們說出「哪一個會留下紀錄」；3/3 才算過 | 驗的是「沒看過規格的人讀不讀得懂」，任何自動化都會偷看到答案 |
