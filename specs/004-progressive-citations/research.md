...
- *尾巴也進鎖，但新觸發時 abort 舊尾巴以提早釋放鎖*：abort 只能擋「還沒送出」的呼叫，已在飛的
  HTTP 沒辦法取消，鎖仍會被握到那次呼叫結束（第一段最多 20 秒〔2026-09-02 前為 15 秒〕、第二段最多 20 秒）。世代比對在鎖外解決同一個問題，且不阻塞。
...
⚠️ **2026-09-02：後半句「第一段維持 15 秒不動」已被推翻。** 第一段也改用獨立常數
`SUGGESTION_STAGE1_CALL_TIMEOUT_MS = 20_000`。推翻的理由不是本文件當時判斷錯誤，而是
當時還沒有的證據：**002 SC-001 的 20 秒門檻配 15 秒逾時，在重試路徑上數學上不可能滿足**
（15 ＋ 1 ＋ 下一次呼叫必然 > 20），2026-09-01 三輪實測 **14/14 個破 20 秒的樣本序列裡都有**
**`retrying`，0 例外**。本文件下一句「001 FR-014 的三數一字不動」**仍然成立** ——
第一段用的是 per-call-site 覆寫，共用常數沒有被動到。理由與代價見 `docs/ARCHITECTURE.md` §8.2b。
