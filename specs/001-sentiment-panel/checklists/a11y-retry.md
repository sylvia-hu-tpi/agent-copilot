...
| CHK016（數字更新） | ⚠️ CHK016 的題目寫「SC-005 的 **10 秒**門檻」，那是 2026-08-26 提問當下的值。**2026-09-01 SC-005 已拆為「摘要 10 秒／情緒 15 秒」**（情緒的批次同時由依序改為並行）。上一列的結論——SC-003 與 SC-005 共用同一次事件送達時間、不重複計時——**不受影響**，因為它講的是計時的起訖點，不是門檻的秒數。題目文字刻意不改：那是當時的提問原文 | `spec.md` SC-005 2026-09-01 修訂、`docs/ARCHITECTURE.md` §8.2b |
