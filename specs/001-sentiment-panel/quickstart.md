...
4. **預期**：3 秒內畫面出現摘要卡與情緒 sparkline 區塊並標示「分析中」（SC-001）；摘要 10 秒內、情緒 15 秒內呈現實質內容（SC-005，⚠️ 2026-09-01 起是兩個不同的門檻；摘要那一半目前實測不通過，屬已知落差，見 `spec.md` SC-005）；情緒 sparkline 的點數對應歷史上「含文字」的客戶發言輪數，純附件輪呈現為時間軸上的中性標記而非分數點（FR-012）。
...
3. **預期**：摘要卡在 10 秒內、情緒 sparkline 在 15 秒內反映新訊息（SC-005 的門檻對冷啟動與增量更新一致）；檢查伺服端送給 `AIProvider.summarize()` 的實際輸入（可在 `MockAIProvider` 加臨時 log 或直接斷點），確認只包含既有摘要與新訊息，**不含**完整歷史（FR-004 的驗收方式是檢查輸入內容，不是量測延遲）。
